...
if(!$reguser){
	...
	if($partuser->subscriptionid==0 && $userintoteam==1){
		...
		//print_object($datasub['MessageDetail']);
		//exit;
		...
		if($subscriptionid!=""){
			...
			if(isset($datasub['MessageDetail']) && !empty($datasub['MessageDetail'])){
				redirect($CFG->wwwroot.'/mod/cloudlabs/view.php?id='.$id.'&errord=1', \core\notification::error($datasub['MessageDetail']));
			} else{
		...
		<div class="col-md-12">
			<center> 
				<form method="POST" name="launchform" value="launchform" action="apicall.php">
					...
					<?php } else {?>
					<a href="view.php?id=<?php echo $id; ?>"><input type="button" name="retry" class="btn btn-primary custombtn fa fa-lock" value="Retry"></a>
					<?php } ?>
				</form>
			</center>
		</div>
