...
require_once($CFG->dirroot.'/mod/amcat/essay_form.php');
...
$dbamcat = $DB->get_record('amcat', array('id' => $cm->instance), '*', MUST_EXIST);
$amcat = new amcat($dbamcat);

require_login($course, false, $cm);
$context = context_module::instance($cm->id);
require_capability('mod/amcat:grade', $context);

$url = new moodle_url('/mod/amcat/essay.php', array('id'=>$id));
if ($mode !== 'display') {
    $url->param('mode', $mode);
}
$PAGE->set_url($url);
...
if ($attemptid > 0) {
    $attempt = $DB->get_record('amcat_attempts', array('id' => $attemptid));
    $answer = $DB->get_record('amcat_answers', array('amcatid' => $amcat->id, 'pageid' => $attempt->pageid));
    ...
    if ($amcat->custom) {
        ...
        $scoreoptions[0] = get_string('nocredit', 'amcat');
        $scoreoptions[1] = get_string('credit', 'amcat');
    }
}
...
switch ($mode) {
    case 'grade':
        ...
        if (empty($attempt)) {
            print_error('cannotfindattempt', 'amcat');
        }
        if (empty($user)) {
            print_error('cannotfinduser', 'amcat');
        }
        if (empty($answer)) {
            print_error('cannotfindanswer', 'amcat');
        }
        break;
    ...
    case 'update':
        ...
        $essayinfo = amcat_page_type_essay::extract_useranswer($attempt->useranswer);
        $essayinfo = file_prepare_standard_editor($essayinfo, 'response', $editoroptions, $context,
                'mod_amcat', 'essay_responses', $attempt->id);
        $mform = new essay_grading_form(null, array('scoreoptions' => $scoreoptions, 'user' => $user));
        $mform->set_data($essayinfo);
        if ($mform->is_cancelled()) {
            redirect("$CFG->wwwroot/mod/amcat/essay.php?id=$cm->id");
        }
        if ($form = $mform->get_data()) {
            if (!$grades = $DB->get_records('amcat_grades', array("amcatid"=>$amcat->id, "userid"=>$attempt->userid), 'completed', '*', $attempt->retry, 1)) {
                print_error('cannotfindgrade', 'amcat');
            }
            ...
            $essayinfo->sent = 0;
            if (!$amcat->custom && $essayinfo->score == 1) {
                $attempt->correct = 1;
            } else {
                $attempt->correct = 0;
            }
            ...
            $gradeinfo = amcat_grade($amcat, $attempt->retry, $attempt->userid);

            // Set and update
            $updategrade = new stdClass();
            $updategrade->id = $grade->id;
            $updategrade->grade = $gradeinfo->grade;
            $DB->update_record('amcat_grades', $updategrade);
            ...
            $event = \mod_amcat\event\essay_assessed::create($params);
            $event->add_record_snapshot('amcat', $dbamcat);
            $event->trigger();

            $amcat->add_message(get_string('changessaved'), 'notifysuccess');

            // update central gradebook
            amcat_update_grades($amcat, $grade->userid);

            redirect(new moodle_url('/mod/amcat/essay.php', array('id'=>$cm->id)));
        } else {
    ...
    case 'email':
        ...
        foreach ($pages as $key=>$page) {
            if ($page->qtype != amcat_PAGE_ESSAY) {
                unset($pages[$key]);
            }
        }
        ...
        if (!$attempts = $DB->get_records_select('amcat_attempts', "pageid $usql".$queryadd, $params)) {
            print_error('nooneansweredthisquestion', 'amcat');
        }
        // Get the answers
        list($answerUsql, $parameters) = $DB->get_in_or_equal(array_keys($pages));
        array_unshift($parameters, $amcat->id);
        if (!$answers = $DB->get_records_select('amcat_answers', "amcatid = ? AND pageid $answerUsql", $parameters, '', 'pageid, score')) {
            print_error('cannotfindanswer', 'amcat');
        }
        ...
        foreach ($attempts as $attempt) {
            ...
            if ($essayinfo->graded && !$essayinfo->sent) {
                ...
                // Set the grade
                $grades = $DB->get_records('amcat_grades', array("amcatid"=>$amcat->id, "userid"=>$attempt->userid), 'completed', '*', $attempt->retry, 1);
                $grade  = current($grades);
                ...
                $currentpage = $amcat->load_page($attempt->pageid);
                ...
                $answer = file_rewrite_pluginfile_urls($essayinfo->answer, 'pluginfile.php', $context->id,
                    'mod_amcat', 'essay_answers', $attempt->id);
                ...
                $a->amcat = format_string($amcat->name, true);

                // Fetch message HTML and plain text formats
                $message  = get_string('essayemailmessage2', 'amcat', $a);
                $plaintext = format_text_email($message, FORMAT_HTML);

                $smallmessage = get_string('essayemailmessagesmall', 'amcat', $a);
                $smallmessage = format_text_email($smallmessage, FORMAT_HTML);

                // Subject
                $subject = get_string('essayemailsubject', 'amcat');
        ...
        $amcat->add_message(get_string('emailsuccess', 'amcat'), 'notifysuccess');
        redirect(new moodle_url('/mod/amcat/essay.php', array('id'=>$cm->id)));
        break;
    case 'display':  // Default view - get the necessary data
    default:
        // Get amcat pages that are essay
        $pages = $amcat->load_all_pages();
        foreach ($pages as $key=>$page) {
            if ($page->qtype != amcat_PAGE_ESSAY) {
                unset($pages[$key]);
        ...
        if (count($pages) > 0) {
            ...
            $sql = "SELECT a.*
                        FROM {amcat_attempts} a
                        JOIN ($esql) ue ON a.userid = ue.id
                        WHERE pageid $usql";
            if ($essayattempts = $DB->get_records_sql($sql, $parameters)) {
                $ufields = user_picture::fields('u');
                // Get all the users who have taken this amcat.
                list($sort, $sortparams) = users_order_by_sql('u');

                $params['amcatid'] = $amcat->id;
                $sql = "SELECT DISTINCT $ufields
                        FROM {user} u
                        JOIN {amcat_attempts} a ON u.id = a.userid
                        JOIN ($esql) ue ON ue.id = a.userid
                        WHERE a.amcatid = :amcatid
                        ORDER BY $sort";
                if (!$users = $DB->get_records_sql($sql, $params)) {
                    $mode = 'none'; // not displaying anything
                    if (!empty($currentgroup)) {
                        $groupname = groups_get_group_name($currentgroup);
                        $amcat->add_message(get_string('noonehasansweredgroup', 'amcat', $groupname));
                    } else {
                        $amcat->add_message(get_string('noonehasanswered', 'amcat'));
                    }
                }
            } else {
                $mode = 'none'; // not displaying anything
            ...
            $mode = 'none'; // not displaying anything
            $amcat->add_message(get_string('noessayquestionsfound', 'amcat'));
        }
        break;
}

$amcatoutput = $PAGE->get_renderer('mod_amcat');
echo $amcatoutput->header($amcat, $cm, 'essay', false, null, get_string('manualgrading', 'amcat'));
...
switch ($mode) {
    case 'display':
        ...
        $table->head = array(get_string('name'), get_string('essays', 'amcat'), get_string('status'),
            get_string('email', 'amcat'));
        ...
        foreach (array_keys($studentessays) as $userid) {
            ...
            // Number of attempts on the amcat
            $attempts = $amcat->count_user_retries($userid);
            ...
            foreach ($studentessays[$userid] as $page => $tries) {
                ...
                foreach($tries as $try) {
                    ...
                    if (count($try) > $amcat->maxattempts) {
                        $essay = $try[$amcat->maxattempts-1];
                    } else {
                        $essay = end($try);
                    }

                    // Start processing the attempt
                    $essayinfo = amcat_page_type_essay::extract_useranswer($essay->useranswer);

                    // link for each essay
                    $url = new moodle_url('/mod/amcat/essay.php', array('id'=>$cm->id,'mode'=>'grade','attemptid'=>$essay->id,'sesskey'=>sesskey()));
                    ...
                    if (!$essayinfo->graded) {
                        ...
                        $status = get_string('notgraded', 'amcat');
                    } elseif (!$essayinfo->sent) {
                        $class = "badge badge-success";
                        $status = get_string('graded', 'amcat');
                    } else {
                        $class = "badge badge-success";
                        $status = get_string('sent', 'amcat');
                    }
            ...
            $url = new moodle_url('/mod/amcat/essay.php', array('id'=>$cm->id,'mode'=>'email','userid'=>$userid,'sesskey'=>sesskey()));
            $emaillink = html_writer::link($url, get_string('emailgradedessays', 'amcat'));
        ...
        $url = new moodle_url('/mod/amcat/essay.php', array('id'=>$cm->id,'mode'=>'email','sesskey'=>sesskey()));
        $emailalllink = html_writer::link($url, get_string('emailallgradedessays', 'amcat'));
