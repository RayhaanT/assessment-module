...
class essay_grading_form extends moodleform {
    ...
    public function definition() {
        ...
        $mform->addElement('header', 'formheader', get_string('question', 'amcat'));
        ...
        $mform->addElement('static', 'question', get_string('question', 'amcat'));
        $mform->addElement('static', 'studentanswer', get_string('studentresponse', 'amcat', fullname($this->_customdata['user'], true)));

        $editoroptions = array('noclean' => true, 'maxfiles' => EDITOR_UNLIMITED_FILES, 'maxbytes' => $CFG->maxbytes);
        $mform->addElement('editor', 'response_editor', get_string('comments', 'amcat'), null, $editoroptions);
        $mform->setType('response', PARAM_RAW);

        $mform->addElement('select', 'score', get_string('essayscore', 'amcat'), $this->_customdata['scoreoptions']);
