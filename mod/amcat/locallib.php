...
/**
 * Local library file for amcat.  These are non-standard functions that are used
 * only by amcat.
 *
 * @package mod_amcat
...
/** This page */
define('amcat_THISPAGE', 0);
/** Next page -> any page not seen before */
define("amcat_UNSEENPAGE", 1);
/** Next page -> any page not answered correctly */
define("amcat_UNANSWEREDPAGE", 2);
/** Jump to Next Page */
define("amcat_NEXTPAGE", -1);
/** End of amcat */
define("amcat_EOL", -9);
/** Jump to an unseen page within a branch and end of branch or end of amcat */
define("amcat_UNSEENBRANCHPAGE", -50);
/** Jump to Previous Page */
define("amcat_PREVIOUSPAGE", -40);
/** Jump to a random page within a branch and end of branch or end of amcat */
define("amcat_RANDOMPAGE", -60);
/** Jump to a random Branch */
define("amcat_RANDOMBRANCH", -70);
/** Cluster Jump */
define("amcat_CLUSTERJUMP", -80);
/** Undefined */
define("amcat_UNDEFINED", -99);

/** amcat_MAX_EVENT_LENGTH = 432000 ; 5 days maximum */
define("amcat_MAX_EVENT_LENGTH", "432000");

/** Answer format is HTML */
define("amcat_ANSWER_HTML", "HTML");

/** Placeholder answer for all other answers. */
define("amcat_OTHER_ANSWERS", "@#wronganswer#@");

//////////////////////////////////////////////////////////////////////////////////////
/// Any other amcat functions go here.  Each of them must have a name that
/// starts with amcat_

/**
 * Checks to see if a amcat_CLUSTERJUMP or
 * a amcat_UNSEENBRANCHPAGE is used in a amcat.
 *
 * This function is only executed when a teacher is
 * checking the navigation for a amcat.
 *
 * @param stdClass $amcat Id of the amcat that is to be checked.
 * @return boolean True or false.
 **/
function amcat_display_teacher_warning($amcat) {
    global $DB;

    // get all of the amcat answers
    $params = array ("amcatid" => $amcat->id);
    if (!$amcatanswers = $DB->get_records_select("amcat_answers", "amcatid = :amcatid", $params)) {
        // no answers, then not using cluster or unseen
        return false;
    }
    // just check for the first one that fulfills the requirements
    foreach ($amcatanswers as $amcatanswer) {
        if ($amcatanswer->jumpto == amcat_CLUSTERJUMP || $amcatanswer->jumpto == amcat_UNSEENBRANCHPAGE) {
            return true;
        }
    }
...
/**
 * Interprets the amcat_UNSEENBRANCHPAGE jump.
 *
 ...
 **/
function amcat_unseen_question_jump($amcat, $user, $pageid) {
    global $DB;

    // get the number of retakes
    if (!$retakes = $DB->count_records("amcat_grades", array("amcatid"=>$amcat->id, "userid"=>$user))) {
        $retakes = 0;
    }

    // get all the amcat_attempts aka what the user has seen
    if ($viewedpages = $DB->get_records("amcat_attempts", array("amcatid"=>$amcat->id, "userid"=>$user, "retry"=>$retakes), "timeseen DESC")) {
    ...
    if ($pageid == amcat_UNSEENBRANCHPAGE) {  // this only happens when a student leaves in the middle of an unseen question within a branch series
    ...
    while ($pageid != 0) { // this condition should never be satisfied... only happens if there are no branch tables above this page
        if ($amcatpages[$pageid]->qtype == amcat_PAGE_BRANCHTABLE) {
            break;
        }
        $pageid = $amcatpages[$pageid]->prevpageid;
    }

    $pagesinbranch = $amcat->get_sub_pages_of($pageid, array(amcat_PAGE_BRANCHTABLE, amcat_PAGE_ENDOFBRANCH));
    ...
    if(count($unseen) == 0) {
        if(isset($pagesinbranch)) {
            ...
            $nextpage = $amcatpages[$pageid]->nextpageid;
        }
        if ($nextpage == 0) {
            return amcat_EOL;
        } else {
 ...
 * @return int Will return the page id of a branch table or end of amcat
 **/
function amcat_unseen_branch_jump($amcat, $userid) {
    global $DB;

    if (!$retakes = $DB->count_records("amcat_grades", array("amcatid"=>$amcat->id, "userid"=>$userid))) {
        $retakes = 0;
    }

    if (!$seenbranches = $amcat->get_content_pages_viewed($retakes, $userid, 'timeseen DESC')) {
        print_error('cannotfindrecords', 'amcat');
    }
    ...
    // this function searches through the amcat pages to find all the branch tables
    // that follow the flagged branch table
    $pageid = $amcatpages[$start]->nextpageid; // move down from the flagged branch table
    $branchtables = array();
    while ($pageid != 0) {  // grab all of the branch table till eol
        if ($amcatpages[$pageid]->qtype == amcat_PAGE_BRANCHTABLE) {
            $branchtables[] = $amcatpages[$pageid]->id;
        }
        $pageid = $amcatpages[$pageid]->nextpageid;
    }
    $unseen = array();
    ...
    if (count($unseen) > 0) {
        ...
        return amcat_EOL;  // has viewed all of the branch tables
    }
}

/**
 * Handles the random jump between a branch table and end of branch or end of amcat (amcat_RANDOMPAGE).
 *
 * @param amcat $amcat
 ...
 **/
function amcat_random_question_jump($amcat, $pageid) {
    global $DB;

    // get the amcat pages
    $params = array ("amcatid" => $amcat->id);
    if (!$amcatpages = $DB->get_records_select("amcat_pages", "amcatid = :amcatid", $params)) {
        print_error('cannotfindpages', 'amcat');
    }
    ...
    if(count($pagesinbranch) == 0) {
        // there are no pages inside the branch, so return the next page
        return $amcatpages[$pageid]->nextpageid;
    } else {
...
/**
 * Calculates a user's grade for a amcat.
 *
 ...
 */
function amcat_grade($amcat, $ntries, $userid = 0) {
    global $USER, $DB;
    ...
    $params = array ("amcatid" => $amcat->id, "userid" => $userid, "retry" => $ntries);
    if ($useranswers = $DB->get_records_select("amcat_attempts",  "amcatid = :amcatid AND
        ...
        // Drop all attempts that go beyond max attempts for the amcat
        foreach ($attemptset as $key => $set) {
            $attemptset[$key] = array_slice($set, 0, $amcat->maxattempts);
        }

        // get only the pages and their answers that the user answered
        list($usql, $parameters) = $DB->get_in_or_equal(array_keys($attemptset));
        array_unshift($parameters, $amcat->id);
        $pages = $DB->get_records_select("amcat_pages", "amcatid = ? AND id $usql", $parameters);
        $answers = $DB->get_records_select("amcat_answers", "amcatid = ? AND pageid $usql", $parameters);

        // Number of pages answered
        $nquestions = count($pages);

        foreach ($attemptset as $attempts) {
            $page = amcat_page::load($pages[end($attempts)->pageid], $amcat);
            if ($amcat->custom) {
        ...
        if ($amcat->custom) {
            ...
            if ($amcat->minquestions and $nquestions < $amcat->minquestions) {
                // Nope, increase number viewed by the amount of unanswered questions
                $total =  $nviewed + ($amcat->minquestions - $nquestions);
            } else {
                $total = $nviewed;
            }
        }
 ...
 * is whether a user has a grade greater than or equal to the amcat setting
 * of displayleftif
 *
 * @param object $amcat amcat object of the current amcat
 * @return boolean 0 if the user cannot see, or $amcat->displayleft to keep displayleft unchanged
 **/
function amcat_displayleftif($amcat) {
    global $CFG, $USER, $DB;

    if (!empty($amcat->displayleftif)) {
        // get the current user's max grade for this amcat
        $params = array ("userid" => $USER->id, "amcatid" => $amcat->id);
        if ($maxgrade = $DB->get_record_sql('SELECT userid, MAX(grade) AS maxgrade FROM {amcat_grades} WHERE userid = :userid AND amcatid = :amcatid GROUP BY userid', $params)) {
            if ($maxgrade->maxgrade < $amcat->displayleftif) {
    ...
    // if we get to here, keep the original state of displayleft amcat setting
    return $amcat->displayleft;
}
 ...
 * @return unknown_type
 */
function amcat_add_fake_blocks($page, $cm, $amcat, $timer = null) {
    $bc = amcat_menu_block_contents($cm->id, $amcat);
    if (!empty($bc)) {
    ...
    $bc = amcat_mediafile_block_contents($cm->id, $amcat);
    if (!empty($bc)) {
        $page->blocks->add_fake_block($bc, $page->blocks->get_default_region());
    }

    if (!empty($timer)) {
        $bc = amcat_clock_block_contents($cm->id, $amcat, $timer, $page);
        if (!empty($bc)) {
            $page->blocks->add_fake_block($bc, $page->blocks->get_default_region());
        }
    }
 ...
 * amcat, return a block_contents that displays it.
 *
 * @param int $cmid Course Module ID for this amcat
 * @param object $amcat Full amcat record object
 * @return block_contents
 **/
function amcat_mediafile_block_contents($cmid, $amcat) {
    global $OUTPUT;
    if (empty($amcat->mediafile)) {
        return null;
    }
    ...
    $options['resizable'] = 1;
    $options['width'] = $amcat->mediawidth;
    $options['height'] = $amcat->mediaheight;

    $link = new moodle_url('/mod/amcat/mediafile.php?id='.$cmid);
    $action = new popup_action('click', $link, 'amcatmediafile', $options);
    $content = $OUTPUT->action_link($link, get_string('mediafilepopup', 'amcat'), $action, array('title'=>get_string('mediafilepopup', 'amcat')));

    $bc = new block_contents();
    $bc->title = get_string('linkedmedia', 'amcat');
...
/**
 * If a timed amcat and not a teacher, then
 ...
 * @return block_contents
 **/
function amcat_clock_block_contents($cmid, $amcat, $timer, $page) {
    // Display for timed amcats and for students only
    $context = context_module::instance($cmid);
    if ($amcat->timelimit == 0 || has_capability('mod/amcat:manage', $context)) {
        return null;
    }

    $content = '<div id="amcat-timer">';
    $content .=  $amcat->time_remaining($timer->starttime);
    $content .= '</div>';

    $clocksettings = array('starttime' => $timer->starttime, 'servertime' => time(), 'testlength' => $amcat->timelimit);
    $page->requires->data_for_js('clocksettings', $clocksettings, true);
    $page->requires->strings_for_js(array('timeisup'), 'amcat');
    $page->requires->js('/mod/amcat/timer.js');
    $page->requires->js_init_call('show_clock');

    $bc = new block_contents();
    $bc->title = get_string('timeremaining', 'amcat');
 ...
 *
 * @param int $cmid Course Module ID for this amcat
 * @param amcat $amcat Full amcat record object
 * @return void
 **/
function amcat_menu_block_contents($cmid, $amcat) {
    global $CFG, $DB;
    ...
    while ($pageid != 0) {
        ...
        if ($page->displayinmenublock && $page->display) {
            if ($page->id == $currentpageid) {
                ...
                $content .= "<li class=\"notselected\"><a href=\"$CFG->wwwroot/mod/amcat/view.php?id=$cmid&amp;pageid=$page->id\">".format_string($page->title,true)."</a></li>\n";
            }
    ...
    $bc = new block_contents();
    $bc->title = get_string('amcatmenu', 'amcat');
...
/**
 * Adds header buttons to the page for the amcat
 *
 * @param object $cm
 * @param object $context
 * @param bool $extraeditbuttons
 * @param int $amcatpageid
 */
function amcat_add_header_buttons($cm, $context, $extraeditbuttons=false, $amcatpageid=null) {
    global $CFG, $PAGE, $OUTPUT;
    if (has_capability('mod/amcat:edit', $context) && $extraeditbuttons) {
        if ($amcatpageid === null) {
            print_error('invalidpageid', 'amcat');
        }
        if (!empty($amcatpageid) && $amcatpageid != amcat_EOL) {
            $url = new moodle_url('/mod/amcat/editpage.php', array(
                'id'       => $cm->id,
                'pageid'   => $amcatpageid,
                'edit'     => 1,
                'returnto' => $PAGE->url->out_as_local_url(false)
            ));
            $PAGE->set_button($OUTPUT->single_button($url, get_string('editpagecontent', 'amcat')));
        }
    }
}
 ...
 */
function amcat_get_media_html($amcat, $context) {
    global $CFG, $PAGE, $OUTPUT;
    require_once("$CFG->libdir/resourcelib.php");

    // get the media file link
    if (strpos($amcat->mediafile, '://') !== false) {
        $url = new moodle_url($amcat->mediafile);
    } else {
        // the timemodified is used to prevent caching problems, instead of '/' we should better read from files table and use sortorder
        $url = moodle_url::make_pluginfile_url($context->id, 'mod_amcat', 'mediafile', $amcat->timemodified, '/', ltrim($amcat->mediafile, '/'));
    }
    $title = $amcat->mediafile;
 ...
 * @return void
 */
function amcat_process_group_deleted_in_course($courseid, $groupid = null) {
    global $DB;
    ...
    if ($groupid) {
        ...
        $sql = "SELECT o.id, o.amcatid
                  FROM {amcat_overrides} o
                  JOIN {amcat} amcat ON amcat.id = o.amcatid
                 WHERE amcat.course = :courseid
                   AND o.groupid = :groupid";
    } else {
        // No groupid, we update all orphaned group overrides for all amcats in course.
        $sql = "SELECT o.id, o.amcatid
                  FROM {amcat_overrides} o
                  JOIN {amcat} amcat ON amcat.id = o.amcatid
    ...
    $DB->delete_records_list('amcat_overrides', 'id', array_keys($records));
}
 ...
 * @since  Moodle 3.3
 */
function amcat_get_overview_report_table_and_data(amcat $amcat, $currentgroup) {
    global $DB, $CFG, $OUTPUT;
    require_once($CFG->dirroot . '/mod/amcat/pagetypes/branchtable.php');

    $context = $amcat->context;
    $cm = $amcat->cm;
    // Count the number of branch and question pages in this amcat.
    $branchcount = $DB->count_records('amcat_pages', array('amcatid' => $amcat->id, 'qtype' => amcat_PAGE_BRANCHTABLE));
    $questioncount = ($DB->count_records('amcat_pages', array('amcatid' => $amcat->id)) - $branchcount);

    // Only load students if there attempts for this amcat.
    $attempts = $DB->record_exists('amcat_attempts', array('amcatid' => $amcat->id));
    $branches = $DB->record_exists('amcat_branch', array('amcatid' => $amcat->id));
    $timer = $DB->record_exists('amcat_timer', array('amcatid' => $amcat->id));
    if ($attempts or $branches or $timer) {
        ...
        $params['a1amcatid'] = $amcat->id;
        $params['b1amcatid'] = $amcat->id;
        $params['c1amcatid'] = $amcat->id;
        $ufields = user_picture::fields('u', $extrafields);
        $sql = "SELECT DISTINCT $ufields
                FROM {user} u
                JOIN (
                    SELECT userid, amcatid FROM {amcat_attempts} a1
                    WHERE a1.amcatid = :a1amcatid
                        UNION
                    SELECT userid, amcatid FROM {amcat_branch} b1
                    WHERE b1.amcatid = :b1amcatid
                        UNION
                    SELECT userid, amcatid FROM {amcat_timer} c1
                    WHERE c1.amcatid = :c1amcatid
                    ) a ON u.id = a.userid
    ...
    if (! $grades = $DB->get_records('amcat_grades', array('amcatid' => $amcat->id), 'completed')) {
        $grades = array();
    }

    if (! $times = $DB->get_records('amcat_timer', array('amcatid' => $amcat->id), 'starttime')) {
        $times = array();
    }
    ...
    $attempts = $DB->get_recordset('amcat_attempts', array('amcatid' => $amcat->id), 'timeseen');
    foreach ($attempts as $attempt) {
        ...
        if (empty($studentdata[$attempt->userid]) || empty($studentdata[$attempt->userid][$attempt->retry])) {
            ...
            // build up the array.
            // this array represents each student and all of their tries at the amcat
    ...
    $branches = $DB->get_recordset('amcat_branch', array('amcatid' => $amcat->id), 'timeseen');
    ...
    // Determine if amcat should have a score.
    if ($branchcount > 0 AND $questioncount == 0) {
        // This amcat only contains content pages and is not graded.
        $data->amcatscored = false;
    } else {
        // This amcat is graded.
        $data->amcatscored = true;
    }
    ...
    $caneditamcat = has_capability('mod/amcat:edit', $context);
    $attemptsheader = get_string('attempts', 'amcat');
    if ($caneditamcat) {
        $selectall = get_string('selectallattempts', 'amcat');
        $deselectall = get_string('deselectallattempts', 'amcat');
        // Build the select/deselect all control.
        $selectallid = 'selectall-attempts';
        $mastercheckbox = new \core\output\checkbox_toggleall('amcat-attempts', true, [
            'id' => $selectallid,
            'name' => $selectallid,
            'value' => 1,
            'label' => $selectall,
    ...
    if ($data->amcatscored) {
        $headers [] = get_string('highscore', 'amcat');
    }
    ...
    // going through each student that has attempted the amcat, so, each student should have something to be displayed
        ...
        if (array_key_exists($student->id, $studentdata)) {
            ...
            foreach ($tries as $try) {
                ...
                if ($try["grade"] !== null) { // if null then not done yet
                    ...
                    $attemptlinkcontents = get_string('attemptinfowithgrade', 'amcat', $attemptdata);

                } else {
                    if ($try["end"]) {
                        // User finished the amcat but has no grade. (Happens when there are only content pages).
                        $timetotake = $try["timeend"] - $try["timestart"];
                        $attemptdata = (object)[
                            'timestart' => userdate($try["timestart"]),
                            'duration' => format_time($timetotake),
                        ];
                        $attemptlinkcontents = get_string('attemptinfonograde', 'amcat', $attemptdata);
                    } else {
                        ...
                        if ($try['timestart'] !== 0) {
                            ...
                            $attemptlinkcontents = get_string("notcompletedwithdate", "amcat", userdate($try["timestart"]));
                        } else {
                            $attemptlinkcontents = get_string("notcompleted", "amcat");
                        }
                        $timetotake = null;
                    }
                }
                $attempturl = new moodle_url('/mod/amcat/report.php', $attempturlparams);
                $attemptlink = html_writer::link($attempturl, $attemptlinkcontents, ['class' => 'amcat-attempt-link']);

                if ($caneditamcat) {
                    $attemptid = 'attempt-' . $try['userid'] . '-' . $try['try'];
                    $attemptname = 'attempts[' . $try['userid'] . '][' . $try['try'] . ']';

                    $checkbox = new \core\output\checkbox_toggleall('amcat-attempts', false, [
                        'id' => $attemptid,
                        'name' => $attemptname,
                        'label' => $attemptlink
                    ]);
                ...
                // Run these lines for the stats only if the user finnished the amcat.
                if ($try["end"]) {
                    // User has completed the amcat.
                    $data->numofattempts++;
            ...
            if ($data->amcatscored) {
                // Add the grade if the amcat is graded.
                $row[] = $bestgrade."%";
            }
 ...
 * @since  Moodle 3.3
 */
function amcat_get_user_detailed_report_data(amcat $amcat, $userid, $attempt) {
    global $DB;
    ...
    $pageid = 0;
    $amcatpages = $amcat->load_all_pages();
    foreach ($amcatpages as $amcatpage) {
        if ($amcatpage->prevpageid == 0) {
            $pageid = $amcatpage->id;
        }
    }
    ...
    $pagestats = array();
    while ($pageid != 0) { // EOL
        $page = $amcatpages[$pageid];
        $params = array ("amcatid" => $amcat->id, "pageid" => $page->id);
        if ($allanswers = $DB->get_records_select("amcat_attempts", "amcatid = :amcatid AND pageid = :pageid", $params, "timeseen")) {
    ...
    while ($pageid != 0) { // EOL
        ...
        if (empty($userid)) {
            ...
            $useranswer = null;
        } elseif ($useranswers = $DB->get_records("amcat_attempts",array("amcatid"=>$amcat->id, "userid"=>$userid, "retry"=>$attempt,"pageid"=>$page->id), "timeseen")) {
    ...
    if (!empty($userid)) {
        $params = array("amcatid"=>$amcat->id, "userid"=>$userid);

        $alreadycompleted = true;

        if (!$grades = $DB->get_records_select("amcat_grades", "amcatid = :amcatid and userid = :userid", $params, "completed", "*", $attempt, 1)) {
            $userstats->grade = -1;
        ...
        if (!$times = $amcat->get_user_timers($userid, 'starttime', '*', $attempt, 1)) {
            $userstats->timetotake = -1;
            $alreadycompleted = false;
        } else {
            $userstats->timetotake = current($times);
            $userstats->timetotake = $userstats->timetotake->amcattime - $userstats->timetotake->starttime;
        }

        if ($alreadycompleted) {
            $userstats->gradeinfo = amcat_grade($amcat, $attempt, $userid);
        }
    }
...
/**
 * Return user's deadline for all amcats in a course, hereby taking into account group and user overrides.
 *
 * @param int $courseid the course id.
 * @return object An object with of all amcatsids and close unixdates in this course,
 * taking into account the most lenient overrides, if existing and 0 if no close date is set.
 */
function amcat_get_user_deadline($courseid) {
    global $DB, $USER;

    // For teacher and manager/admins return amcat's deadline.
    if (has_capability('moodle/course:update', context_course::instance($courseid))) {
        $sql = "SELECT amcat.id, amcat.deadline AS userdeadline
                  FROM {amcat} amcat
                 WHERE amcat.course = :courseid";
    ...
    $sql = "SELECT a.id,
              ...
              FROM (
                      SELECT amcat.id as amcatid,
                             MAX(leo.deadline) AS userclose, MAX(qgo.deadline) AS groupclose
                        FROM {amcat} amcat
                   LEFT JOIN {amcat_overrides} leo on amcat.id = leo.amcatid AND leo.userid = :userid
                   LEFT JOIN {groups_members} gm ON gm.userid = :useringroupid
                   LEFT JOIN {amcat_overrides} qgo on amcat.id = qgo.amcatid AND qgo.groupid = gm.groupid
                       WHERE amcat.course = :courseid
                    GROUP BY amcat.id
                   ) v
              JOIN {amcat} a ON a.id = v.amcatid";
 ...
 */
abstract class amcat_add_page_form_base extends moodleform {

    /**
     * This is the classic define that is used to identify this pagetype.
     * Will be one of amcat_*
     * @var int
     */
    public $qtype;
    ...
    public final function definition() {
        ...
        if ($this->qtypestring != 'selectaqtype') {
            if ($this->_customdata['edit']) {
                $mform->addElement('header', 'qtypeheading', get_string('edit'. $this->qtypestring, 'amcat'));
            } else {
                $mform->addElement('header', 'qtypeheading', get_string('add'. $this->qtypestring, 'amcat'));
            }
        }
        ...
        if ($this->_customdata['edit'] === true) {
            ...
            $this->add_action_buttons(get_string('cancel'), get_string('savepage', 'amcat'));
        } else if ($this->qtype === 'questiontype') {
            $this->add_action_buttons(get_string('cancel'), get_string('addaquestionpage', 'amcat'));
        } else {
            $this->add_action_buttons(get_string('cancel'), get_string('savepage', 'amcat'));
        }
    }
     ...
     */
    protected final function add_jumpto($name, $label=null, $selected=amcat_NEXTPAGE) {
        $title = get_string("jump", "amcat");
        if ($label === null) {
            $label = $title;
        }
        if (is_int($name)) {
        ...
        $this->_form->addHelpButton($name, 'jumps', 'amcat');
    }
    ...
    protected final function add_answer($count, $label = null, $required = false, $format= '') {
        if ($label === null) {
            $label = get_string('answer', 'amcat');
        }

        if ($format == amcat_ANSWER_HTML) {
    ...
    protected final function add_response($count, $label = null, $required = false) {
        if ($label === null) {
            $label = get_string('response', 'amcat');
        }
...
/**
 * Class representation of a amcat
 *
 * This class is used the interact with, and manage a amcat once instantiated.
 * If you need to fetch a amcat object you can do so by calling
 *
 * <code>
 * amcat::load($amcatid);
 * // or
 * $amcatrecord = $DB->get_record('amcat', $amcatid);
 * $amcat = new amcat($amcatrecord);
 * </code>
 *
 * The class itself extends amcat_base as all classes within the amcat module should
 *
 * These properties are from the database
 * @property int $id The id of this amcat
 * @property int $course The ID of the course this amcat belongs to
 * @property string $name The name of this amcat
 * @property int $practice Flag to toggle this as a practice amcat
 ...
 * @property int $dependency ID of another amcat this amcat is dependent on
 * @property string $conditions Conditions of the amcat dependency
 ...
 * @property int $maxpages Maximum number of pages this amcat can contain
 * @property int $retake Flag to allow users to retake a amcat
 * @property int $activitylink Relate this amcat to another amcat
 ...
 * @property int $progressbar Flag to toggle display of a amcat progress bar
 * @property int $available Timestamp of when this amcat becomes available
 * @property int $deadline Timestamp of when this amcat is no longer available
 * @property int $timemodified Timestamp when amcat was last modified
 * @property int $allowofflineattempts Whether to allow the amcat to be attempted offline in the mobile app
 *
 * These properties are calculated
 * @property int $firstpageid Id of the first page of this amcat (prevpageid=0)
 * @property int $lastpageid Id of the last page of this amcat (nextpageid=0)
 *
 * @copyright  2009 Sam Hemelryk
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class amcat extends amcat_base {

    /**
     * The id of the first page (where prevpageid = 0) gets set and retrieved by
     * {@see get_firstpageid()} by directly calling <code>$amcat->firstpageid;</code>
     * @var int
     */
    protected $firstpageid = null;
    /**
     * The id of the last page (where nextpageid = 0) gets set and retrieved by
     * {@see get_lastpageid()} by directly calling <code>$amcat->lastpageid;</code>
     * @var int
     */
    protected $lastpageid = null;
    /**
     * An array used to cache the pages associated with this amcat after the first
     * time they have been loaded.
     * A note to developers: If you are going to be working with MORE than one or
     * two pages from a amcat you should probably call {@see $amcat->load_all_pages()}
     * in order to save excess database queries.
     * @var array An array of amcat_page objects
     */
    protected $pages = array();
    /**
     * Flag that gets set to true once all of the pages associated with the amcat
     * have been loaded.
     * @var bool
     */
    ...
    /**
     * Course module object gets set and retrieved by directly calling <code>$amcat->cm;</code>
     * @see get_cm()
     * @var stdClass
     */
    protected $cm = null;
    ...
    /**
     * Course object gets set and retrieved by directly calling <code>$amcat->courserecord;</code>
    ...
    /**
     * Context object gets set and retrieved by directly calling <code>$amcat->context;</code>
     * @see get_context()
     * @var stdClass
     */
    ...
    /**
     * Simply generates a amcat object given an array/object of properties
     * Overrides {@see amcat_base->create()}
     * @static
     * @param object|array $properties
     * @return amcat
     */
    public static function create($properties) {
        return new amcat($properties);
    }

    /**
     * Generates a amcat object from the database given its id
     * @static
     * @param int $amcatid
     * @return amcat
     */
    public static function load($amcatid) {
        global $DB;

        if (!$amcat = $DB->get_record('amcat', array('id' => $amcatid))) {
            print_error('invalidcoursemodule');
        }
        return new amcat($amcat);
    }

    /**
     * Deletes this amcat from the database
     */
    public function delete() {
        ...
        $cm = get_coursemodule_from_instance('amcat', $this->properties->id, $this->properties->course);
        $context = context_module::instance($cm->id);

        $this->delete_all_overrides();

        grade_update('mod/amcat', $this->properties->course, 'mod', 'amcat', $this->properties->id, 0, null, array('deleted'=>1));

        // We must delete the module record after we delete the grade item.
        $DB->delete_records("amcat", array("id"=>$this->properties->id));
        $DB->delete_records("amcat_pages", array("amcatid"=>$this->properties->id));
        $DB->delete_records("amcat_answers", array("amcatid"=>$this->properties->id));
        $DB->delete_records("amcat_attempts", array("amcatid"=>$this->properties->id));
        $DB->delete_records("amcat_grades", array("amcatid"=>$this->properties->id));
        $DB->delete_records("amcat_timer", array("amcatid"=>$this->properties->id));
        $DB->delete_records("amcat_branch", array("amcatid"=>$this->properties->id));
        if ($events = $DB->get_records('event', array("modulename"=>'amcat', "instance"=>$this->properties->id))) {
    ...
    /**
     * Deletes a amcat override from the database and clears any corresponding calendar events
     *
    ...
    public function delete_override($overrideid) {
        ...
        $override = $DB->get_record('amcat_overrides', array('id' => $overrideid), '*', MUST_EXIST);
        ...
        $DB->delete_records('amcat_overrides', array('id' => $overrideid));
        ...
        $params = array(
            ...
            'context' => context_module::instance($cm->id),
            'other' => array(
                'amcatid' => $override->amcatid
            )
        );
        // Determine which override deleted event to fire.
        if (!empty($override->userid)) {
            $params['relateduserid'] = $override->userid;
            $event = \mod_amcat\event\user_override_deleted::create($params);
        } else {
            $params['other']['groupid'] = $override->groupid;
            $event = \mod_amcat\event\group_override_deleted::create($params);
        }

        // Trigger the override deleted event.
        $event->add_record_snapshot('amcat_overrides', $override);
        $event->trigger();
    ...
    /**
     * Deletes all amcat overrides from the database and clears any corresponding calendar events
     */
    public function delete_all_overrides() {
        global $DB;

        $overrides = $DB->get_records('amcat_overrides', array('amcatid' => $this->properties->id), 'id');
    ...
    public function is_participant($userid) {
        return is_enrolled($this->get_context(), $userid, 'mod/amcat:view', $this->show_only_active_users());
    }
    ...
    /**
     * Updates the amcat properties with override information for a user.
     *
     * Algorithm:  For each amcat setting, if there is a matching user-specific override,
     ...
     *   amcat->extrapasswords will contain an array of strings giving the remaining
     *   passwords.
     *
    ...
    public function update_effective_access($userid) {
        ...
        $override = $DB->get_record('amcat_overrides', array('amcatid' => $this->properties->id, 'userid' => $userid));
        ...
        if (!empty($groupings[0])) {
            ...
            $sql = "SELECT * FROM {amcat_overrides}
                    WHERE groupid $extra AND amcatid = ?";
        ...
        // Special handling of amcat usepassword and password.
    ...
    protected function get_messages() {
        ...
        $messages = array();
        if (!empty($SESSION->amcat_messages) && is_array($SESSION->amcat_messages) && array_key_exists($this->properties->id, $SESSION->amcat_messages)) {
            $messages = $SESSION->amcat_messages[$this->properties->id];
            unset($SESSION->amcat_messages[$this->properties->id]);
        }
    ...
    public function get_attempts($retries, $correct=false, $pageid=null, $userid=null) {
        global $USER, $DB;
        $params = array("amcatid"=>$this->properties->id, "userid"=>$userid, "retry"=>$retries);
        if ($correct) {
            $params['correct'] = 1;
        }
        if ($pageid !== null) {
        ...
        return $DB->get_records('amcat_attempts', $params, 'timeseen ASC');
    }


    /**
     * Get a list of content pages (formerly known as branch tables) viewed in the amcat for the given user during an attempt.
     *
     * @param  int $amcatattempt the amcat attempt number (also known as retries)
     ...
     * @since  Moodle 3.3
     */
    public function get_content_pages_viewed($amcatattempt, $userid = null, $sort = '', $fields = '*') {
        global $USER, $DB;

        if ($userid === null) {
            $userid = $USER->id;
        }
        $conditions = array("amcatid" => $this->properties->id, "userid" => $userid, "retry" => $amcatattempt);
        return $DB->get_records('amcat_branch', $conditions, $sort, $fields);
    }

    /**
     * Returns the first page for the amcat or false if there isn't one.
     *
     * This method should be called via the magic method __get();
     * <code>
     * $firstpage = $amcat->firstpage;
     * </code>
     *
     * @return amcat_page|bool Returns the amcat_page specialised object or false
     */
    ...
    /**
     * Returns the last page for the amcat or false if there isn't one.
     *
     * This method should be called via the magic method __get();
     * <code>
     * $lastpage = $amcat->lastpage;
     * </code>
     *
     * @return amcat_page|bool Returns the amcat_page specialised object or false
     */
    ...
    /**
     * Returns the id of the first page of this amcat. (prevpageid = 0)
     * @return int
     */
    protected function get_firstpageid() {
        ...
        if ($this->firstpageid == null) {
            if (!$this->loadedallpages) {
                $firstpageid = $DB->get_field('amcat_pages', 'id', array('amcatid'=>$this->properties->id, 'prevpageid'=>0));
                if (!$firstpageid) {
    ...
    /**
     * Returns the id of the last page of this amcat. (nextpageid = 0)
     * @return int
     */
    public function get_lastpageid() {
        ...
        if ($this->lastpageid == null) {
            if (!$this->loadedallpages) {
                $lastpageid = $DB->get_field('amcat_pages', 'id', array('amcatid'=>$this->properties->id, 'nextpageid'=>0));
                if (!$lastpageid) {
                    print_error('cannotfindlastpage', 'amcat');
                }
    ...
    public function get_next_page($nextpageid) {
        ...
        if ($this->properties->nextpagedefault) {
            ...
            $nretakes = $DB->count_records("amcat_grades", array("amcatid" => $this->properties->id, "userid" => $USER->id));
            shuffle($allpages);
            $found = false;
            if ($this->properties->nextpagedefault == amcat_UNSEENPAGE) {
                foreach ($allpages as $nextpage) {
                    if (!$DB->count_records("amcat_attempts", array("pageid" => $nextpage->id, "userid" => $USER->id, "retry" => $nretakes))) {
                        $found = true;
                        break;
                    }
                }
            } elseif ($this->properties->nextpagedefault == amcat_UNANSWEREDPAGE) {
                foreach ($allpages as $nextpage) {
                    if (!$DB->count_records("amcat_attempts", array('pageid' => $nextpage->id, 'userid' => $USER->id, 'correct' => 1, 'retry' => $nretakes))) {
                        $found = true;
                        break;
                    }
                }
            ...
            if ($found) {
                if ($this->properties->maxpages) {
                    // check number of pages viewed (in the amcat)
                    if ($DB->count_records("amcat_attempts", array("amcatid" => $this->properties->id, "userid" => $USER->id, "retry" => $nretakes)) >= $this->properties->maxpages) {
                        return amcat_EOL;
                    }
                }
                return $nextpage->id;
    ...
    /**
     * Sets a message against the session for this amcat that will displayed next
     * time the amcat processes messages
     *
    ...
    public function add_message($message, $class="notifyproblem", $align='center') {
        ...
        if (empty($SESSION->amcat_messages) || !is_array($SESSION->amcat_messages)) {
            $SESSION->amcat_messages = array();
            $SESSION->amcat_messages[$this->properties->id] = array();
        } else if (!array_key_exists($this->properties->id, $SESSION->amcat_messages)) {
            $SESSION->amcat_messages[$this->properties->id] = array();
        }

        $SESSION->amcat_messages[$this->properties->id][] = array($message, $class, $align);

        return true;
    }

    /**
     * Check if the amcat is accessible at the present time
     * @return bool True if the amcat is accessible, false otherwise
     */
    ...
    /**
     * Starts the amcat time for the current user
    ...
    public function start_timer() {
        ...
        // Trigger amcat started event.
        $event = \mod_amcat\event\amcat_started::create(array(
        ...
        $USER->startamcat[$this->properties->id] = true;

        $timenow = time();
        $startamcat = new stdClass;
        $startamcat->amcatid = $this->properties->id;
        $startamcat->userid = $USER->id;
        $startamcat->starttime = $timenow;
        $startamcat->amcattime = $timenow;
        if (WS_SERVER) {
            $startamcat->timemodifiedoffline = $timenow;
        }
        $DB->insert_record('amcat_timer', $startamcat);
        if ($this->properties->timelimit) {
            $this->add_message(get_string('timelimitwarning', 'amcat', format_time($this->properties->timelimit)), 'center');
        }
        return true;
    }
    ...
    public function update_timer($restart=false, $continue=false, $endreached =false) {
        ...
        if ($restart) {
            if ($continue) {
                ...
                $timer->starttime = time() - ($timer->amcattime - $timer->starttime);

                // Trigger amcat resumed event.
                $event = \mod_amcat\event\amcat_resumed::create(array(
                    'objectid' => $this->properties->id,
                    'context' => context_module::instance($cm->id),
                    'courseid' => $this->properties->course
                ));
                ...
                // Trigger amcat restarted event.
                $event = \mod_amcat\event\amcat_restarted::create(array(
                    'objectid' => $this->properties->id,
                    'context' => context_module::instance($cm->id),
                    'courseid' => $this->properties->course
                ));
    ...
    public function stop_timer() {
        global $USER, $DB;
        unset($USER->startamcat[$this->properties->id]);

        $cm = get_coursemodule_from_instance('amcat', $this->properties()->id, $this->properties()->course,
            false, MUST_EXIST);

        // Trigger amcat ended event.
        $event = \mod_amcat\event\amcat_ended::create(array(
            'objectid' => $this->properties()->id,
            'context' => context_module::instance($cm->id),
            'courseid' => $this->properties()->course
        ));
    ...
    /**
     * Checks to see if the amcat has pages
     */
    public function has_pages() {
        global $DB;
        $pagecount = $DB->count_records('amcat_pages', array('amcatid'=>$this->properties->id));
        return ($pagecount>0);
    }
            ...
            if ($modname) {
                ...
                if ($instancename) {
                    return html_writer::link(new moodle_url('/mod/'.$modname.'/view.php',
                        ...
                        'amcat', $instancename), array('class' => 'centerpadded amcatbutton standardbutton p-r-1'));
                }
            }
        }
        return '';
     ...
     * amcat_page object OR as a generic amcat_page.
     ...
     *
     * @param int $pageid
     * @return amcat_page A amcat_page object or an object that extends it
     */
    public function load_page($pageid) {
        if (!array_key_exists($pageid, $this->pages)) {
            $manager = amcat_page_type_manager::get($this);
    ...
    /**
     * Loads ALL of the pages for this amcat
     *
     * @return array An array containing all pages from this amcat
     */
    ...
    public function duplicate_page($pageid) {
        ...
        $newamcatpage = amcat_page::create($properties, $this, $context, $PAGE->course->maxbytes);
        $newanswers = $newamcatpage->get_answers();
        // Copy over the file areas as well.
        $this->copy_page_files('page_contents', $pageid, $newamcatpage->id, $context->id);
        $j = 0;
    ...
    protected function copy_page_files($filearea, $itemid, $newitemid, $contextid) {
        $fs = get_file_storage();
        $files = $fs->get_area_files($contextid, 'mod_amcat', $filearea, $itemid);
    ...
    public function jumpto_is_correct($pageid, $jumpto) {
        ...
        if (!$jumpto) {
            // same page
            return false;
        } elseif ($jumpto == amcat_NEXTPAGE) {
            return true;
        } elseif ($jumpto == amcat_UNSEENBRANCHPAGE) {
            return true;
        } elseif ($jumpto == amcat_RANDOMPAGE) {
            return true;
        } elseif ($jumpto == amcat_CLUSTERJUMP) {
            return true;
        } elseif ($jumpto == amcat_EOL) {
            return true;
        }
    ...
    /**
     * Returns the time a user has remaining on this amcat
    ...
    /**
     * Interprets amcat_CLUSTERJUMP jumpto value.
     *
     * This will select a page randomly
     * and the page selected will be inbetween a cluster page and end of clutter or end of amcat
    ...
    public function cluster_jump($pageid, $userid=null) {
        ...
        // get the number of retakes
        if (!$retakes = $DB->count_records("amcat_grades", array("amcatid"=>$this->properties->id, "userid"=>$userid))) {
            $retakes = 0;
        }
        // get all the amcat_attempts aka what the user has seen
        $seenpages = array();
        ...
        $clusterpages = array();
        $clusterpages = $this->get_sub_pages_of($pageid, array(amcat_PAGE_ENDOFCLUSTER));
        $unseen = array();
        foreach ($clusterpages as $key=>$cluster) {
            ...
            if ($this->is_sub_page_of_type($cluster->id,
                    array(amcat_PAGE_BRANCHTABLE), array(amcat_PAGE_ENDOFBRANCH, amcat_PAGE_CLUSTER))
                    || $cluster->qtype == amcat_PAGE_ENDOFBRANCH) {
                unset($clusterpages[$key]);
            } else if ($cluster->qtype == amcat_PAGE_BRANCHTABLE) {
                // If branchtable, check to see if any pages inside have been viewed.
                $branchpages = $this->get_sub_pages_of($cluster->id, array(amcat_PAGE_BRANCHTABLE, amcat_PAGE_ENDOFBRANCH));
                $flag = true;
        ...
        if (count($unseen) > 0) {
            ...
            if ($nextpage->qtype == amcat_PAGE_BRANCHTABLE) {
                // if branch table, then pick a random page inside of it
                $branchpages = $this->get_sub_pages_of($nextpage->id, array(amcat_PAGE_BRANCHTABLE, amcat_PAGE_ENDOFBRANCH));
            ...
            if (end($clusterpages)->nextpageid == 0) {
                ...
                while ($clusterendid != 0) { // This condition should not be satisfied... should be an end of cluster page.
                    if ($amcatpages[$clusterendid]->qtype == amcat_PAGE_ENDOFCLUSTER) {
                        break;
                    }
                    $clusterendid = $amcatpages[$clusterendid]->nextpageid;
                }
                $exitjump = $DB->get_field("amcat_answers", "jumpto", array("pageid" => $clusterendid, "amcatid" => $this->properties->id));
                if ($exitjump == amcat_NEXTPAGE) {
                    $exitjump = $amcatpages[$clusterendid]->nextpageid;
                }
                if ($exitjump == 0) {
                    return amcat_EOL;
                } else if (in_array($exitjump, array(amcat_EOL, amcat_PREVIOUSPAGE))) {
                    return $exitjump;
                } else {
                    if (!array_key_exists($exitjump, $amcatpages)) {
                        $found = false;
                        foreach ($amcatpages as $page) {
                            if ($page->id === $clusterendid) {
                                $found = true;
                            } else if ($page->qtype == amcat_PAGE_ENDOFCLUSTER) {
                                $exitjump = $DB->get_field("amcat_answers", "jumpto", array("pageid" => $page->id, "amcatid" => $this->properties->id));
                                if ($exitjump == amcat_NEXTPAGE) {
                                    $exitjump = $amcatpages[$page->id]->nextpageid;
                                }
                                break;
                            }
                        }
                    ...
                    if ($amcatpages[$exitjump]->qtype == amcat_PAGE_CLUSTER) {
     ...
     *
     * @param int $pageid
     * @param array $ends An array of amcat_PAGE_* types that signify an end of
     *               the subtype
     * @return array An array of specialised amcat_page objects
     */
    public function get_sub_pages_of($pageid, array $ends) {
        $amcatpages = $this->load_all_pages();
        $pageid = $amcatpages[$pageid]->nextpageid;  // move to the first page after the branch table
        $pages = array();

        while (true) {
            if ($pageid == 0 || in_array($amcatpages[$pageid]->qtype, $ends)) {
                break;
            }
            $pages[] = $amcatpages[$pageid];
            $pageid = $amcatpages[$pageid]->nextpageid;
        }
    ...
    public function resort_pages($pageid, $after) {
        ...
        if (!array_key_exists($pageid, $pages) || ($after!=0 && !array_key_exists($after, $pages))) {
            print_error('cannotfindpages', 'amcat', "$CFG->wwwroot/mod/amcat/edit.php?id=$cm->id");
        }
        ...
        if (!empty($pageupdated)) {
            ...
            $event = \mod_amcat\event\page_moved::create($eventparams);
            $event->trigger();
        }
    ...
    /**
     * Return the amcat context object.
     *
    ...
    /**
     * Set the amcat course module object.
     *
    ...
    /**
     * Return the amcat course module object.
     *
    ...
    public function get_cm() {
        if ($this->cm == null) {
            $this->cm = get_coursemodule_from_instance('amcat', $this->properties->id);
        }
        return $this->cm;
    }

    /**
     * Set the amcat course object.
     *
    ...
    /**
     * Return the amcat course object.
     *
    ...
    /**
     * Check if the user can manage the amcat activity.
     *
     * @return bool true if the user can manage the amcat
     * @since  Moodle 3.3
     */
    public function can_manage() {
        return has_capability('mod/amcat:manage', $this->get_context());
    }
    ...
    public function get_time_restriction_status() {
        ...
        if (!$this->is_accessible()) {
            if ($this->properties->deadline != 0 && time() > $this->properties->deadline) {
                $status = ['reason' => 'amcatclosed', 'time' => $this->properties->deadline];
            } else {
                $status = ['reason' => 'amcatopen', 'time' => $this->properties->available];
            }
    ...
    public function get_dependencies_restriction_status() {
        ...
        if ($dependentamcat = $DB->get_record('amcat', array('id' => $this->properties->dependency))) {
            // amcat exists, so we can proceed.
            ...
            if ($conditions->timespent) {
                $timespent = false;
                if ($attempttimes = $DB->get_records('amcat_timer', array("userid" => $USER->id, "amcatid" => $dependentamcat->id))) {
                    // Go through all the times and test to see if any of them satisfy the condition.
                    foreach ($attempttimes as $attempttime) {
                        $duration = $attempttime->amcattime - $attempttime->starttime;
                ...
                if (!$timespent) {
                    $errors[] = get_string('timespenterror', 'amcat', $conditions->timespent);
                }
            }
            // Check for the gradebetterthan condition.
            if ($conditions->gradebetterthan) {
                $gradebetterthan = false;
                if ($studentgrades = $DB->get_records('amcat_grades', array("userid" => $USER->id, "amcatid" => $dependentamcat->id))) {
                ...
                if (!$gradebetterthan) {
                    $errors[] = get_string('gradebetterthanerror', 'amcat', $conditions->gradebetterthan);
                }
            }
            // Check for the completed condition.
            if ($conditions->completed) {
                if (!$DB->count_records('amcat_grades', array('userid' => $USER->id, 'amcatid' => $dependentamcat->id))) {
                    $errors[] = get_string('completederror', 'amcat');
                }
            }
            if (!empty($errors)) {
                return (object) ['errors' => $errors, 'dependentamcat' => $dependentamcat];
            }
        }
        return false;
    }
    ...
    /**
     * Check if the amcat is in review mode. (The user already finished it and retakes are not allowed).
     *
    ...
    public function is_in_review_mode() {
        ...
        $userhasgrade = $DB->count_records("amcat_grades", array("amcatid" => $this->properties->id, "userid" => $USER->id));
     ...
     *
     * @param int $retriescount the number of retries for the amcat (the last retry number).
     * @return mixed false if the user didn't see the amcat or the last page id
     */
    public function get_last_page_seen($retriescount) {
        ...
        if (!empty($allattempts)) {
            ...
            $jumpto = $DB->get_field('amcat_answers', 'jumpto', array('id' => $attempt->answerid));
            ...
            } else if ($jumpto == amcat_NEXTPAGE) {
                $lastpageseen = $this->get_next_page($attemptpage->nextpageid);
            } else if ($jumpto == amcat_CLUSTERJUMP) {
    ...
    /**
     * Return the number of retries in a amcat for a given user.
     *
     * @param  int $userid the user id
    ...
    public function count_user_retries($userid) {
        ...
        return $DB->count_records('amcat_grades', array("amcatid" => $this->properties->id, "userid" => $userid));
    }
    ...
    public function left_during_timed_session($retriescount) {
        ...
        $conditions = array('amcatid' => $this->properties->id, 'userid' => $USER->id, 'retry' => $retriescount);
        return $DB->count_records('amcat_attempts', $conditions) > 0 || $DB->count_records('amcat_branch', $conditions) > 0;
    }
    ...
    public function set_module_viewed() {
        ...
        $event = \mod_amcat\event\course_module_viewed::create(array(
            'objectid' => $this->properties->id,
     ...
     * @return array    list of timers for the given user in the amcat
     * @since  Moodle 3.3
     */
    public function get_user_timers($userid = null, $sort = '', $fields = '*', $limitfrom = 0, $limitnum = 0) {
        ...
        $params = array('amcatid' => $this->properties->id, 'userid' => $userid);
        return $DB->get_records('amcat_timer', $params, $sort, $fields, $limitfrom, $limitnum);
    }

    /**
     * Check if the user is out of time in a timed amcat.
     *
    ...
    public function check_time($timer) {
        if ($this->properties->timelimit) {
            ...
            if ($timeleft <= 0) {
                // Out of time.
                $this->add_message(get_string('eolstudentoutoftime', 'amcat'));
                return false;
            } else if ($timeleft < 60) {
                // One minute warning.
                $this->add_message(get_string('studentoneminwarning', 'amcat'));
            }
        }
        return true;
    }
     ...
     *
     * @param amcat_page $page page object
     * @param reviewmode $bool whether we are in review mode or not
     * @since  Moodle 3.3
     */
    public function add_messages_on_page_view(amcat_page $page, $reviewmode) {
        global $DB, $USER;

        if (!$this->can_manage()) {
            if ($page->qtype == amcat_PAGE_BRANCHTABLE && $this->properties->minquestions) {
                // Tell student how many questions they have seen, how many are required and their grade.
                $ntries = $DB->count_records("amcat_grades", array("amcatid" => $this->properties->id, "userid" => $USER->id));
                $gradeinfo = amcat_grade($this, $ntries);
                if ($gradeinfo->attempts) {
                    if ($gradeinfo->nquestions < $this->properties->minquestions) {
                        ...
                        $this->add_message(get_string('numberofpagesviewednotice', 'amcat', $a));
                    }

                    if (!$reviewmode && $this->properties->ongoing) {
                        $this->add_message(get_string("numberofcorrectanswers", "amcat", $gradeinfo->earned), 'notify');
                        if ($this->properties->grade != GRADE_TYPE_NONE) {
                            ...
                            $this->add_message(get_string('yourcurrentgradeisoutof', 'amcat', $a), 'notify');
                        }
                    }
                }
            }
            ...
            if ($this->properties->timelimit) {
                $this->add_message(get_string('teachertimerwarning', 'amcat'));
            }
            if (amcat_display_teacher_warning($this)) {
                ...
                $warningvars->cluster = get_string('clusterjump', 'amcat');
                $warningvars->unseen = get_string('unseenpageinbranch', 'amcat');
                $this->add_message(get_string('teacherjumpwarning', 'amcat', $warningvars));
            }
        }
    }

    /**
     * Get the ongoing score message for the user (depending on the user permission and amcat settings).
     *
    ...
    public function get_ongoing_score_message() {
        ...
        if (has_capability('mod/amcat:manage', $context)) {
            return get_string('teacherongoingwarning', 'amcat');
        } else {
            $ntries = $DB->count_records("amcat_grades", array("amcatid" => $this->properties->id, "userid" => $USER->id));
            ...
            if ($this->properties->custom) {
                ...
                return get_string("ongoingcustom", "amcat", $a);
            } else {
                $a->correct = $gradeinfo->earned;
                $a->viewed = $gradeinfo->attempts;
                return get_string("ongoingnormal", "amcat", $a);
            }
        }
    }

    /**
     * Calculate the progress of the current user in the amcat.
     *
    ...
    public function calculate_progress() {
        ...
        if (!$ntries = $DB->count_records("amcat_grades", array("amcatid" => $this->properties->id, "userid" => $USER->id))) {
     ...
     * @param  mod_amcat_renderer $amcatoutput the amcat output rendered
     ...
     * @since  Moodle 3.3
     */
    public function prepare_page_and_contents($pageid, $amcatoutput, $reviewmode, $redirect = true) {
        global $USER, $CFG;
        ...
        if (is_array($page->answers) && count($page->answers) > 0) {
            ...
            if (isset($USER->modattempts[$this->properties->id])) {
                ...
                if (!$attempts = $this->get_attempts($retries - 1, false, $page->id)) {
                    throw new moodle_exception('cannotfindpreattempt', 'amcat');
                }
                $attempt = end($attempts);
            ...
            $amcatcontent = $amcatoutput->display_page($this, $page, $attempt);
        } else {
            require_once($CFG->dirroot . '/mod/amcat/view_form.php');
            $data = new stdClass;
            ...
            $mform = new amcat_page_without_answers($CFG->wwwroot.'/mod/amcat/continue.php', $customdata);
            $mform->set_data($data);
            ob_start();
            $mform->display();
            $amcatcontent = ob_get_contents();
            ob_end_clean();
        }

        return array($page->id, $page, $amcatcontent);
    }

    /**
     * This returns a real page id to jump to (or amcat_EOL) after processing page responses.
     *
     * @param  amcat_page $page      amcat page
     * @param  int         $newpageid the new page id
     * @return int the real page to jump to (or end of amcat)
     * @since  Moodle 3.3
     */
    public function calculate_new_page_on_jump(amcat_page $page, $newpageid) {
        global $USER, $DB;
        ...
        if (isset($USER->modattempts[$this->properties->id])) {
            ...
            if ($USER->modattempts[$this->properties->id]->pageid == $page->id && $page->nextpageid == 0) {
                ...
                $attempts = $DB->get_records("amcat_attempts", array("amcatid" => $this->properties->id, "userid" => $USER->id, "retry" => $nretakes), "timeseen", "id, pageid");
                $found = false;
                $temppageid = 0;
        ...
        } else if ($newpageid != amcat_CLUSTERJUMP && $page->id != 0 && $newpageid > 0) {
        ...
        } else if ($newpageid == amcat_UNSEENBRANCHPAGE) {
            if ($canmanage) {
                ...
                $newpageid = amcat_unseen_question_jump($this, $USER->id, $page->id);
            }
        } else if ($newpageid == amcat_PREVIOUSPAGE) {
            $newpageid = $page->prevpageid;
        } else if ($newpageid == amcat_RANDOMPAGE) {
            $newpageid = amcat_random_question_jump($this, $page->id);
        } else if ($newpageid == amcat_CLUSTERJUMP) {
            if ($canmanage) {
            ...
            $newpageid = $page->id;
        } else if ($newpageid == amcat_NEXTPAGE) {
     ...
     * @since  Moodle 3.3
     */
    public function process_page_responses(amcat_page $page) {
        $context = $this->get_context();
     ...
     * @since  Moodle 3.3
     */
    public function add_messages_on_page_process(amcat_page $page, $result, $reviewmode) {
        ...
        if ($result->attemptsremaining != 0 && $this->properties->review && !$reviewmode) {
            $this->add_message(get_string('attemptsremaining', 'amcat', $result->attemptsremaining));
        }
    }

    /**
     * Process and return all the information for the end of amcat page.
     *
    ...
    public function process_eol_page($outoftime) {
        ...
        if ($this->properties->custom && !$canmanage) {
            ...
            // number specified in the minimum questions setting - which is done in amcat_grade().
            // Get the number of answers given.
            if ($gradeinfo->nquestions < $this->properties->minquestions) {
                $data->gradeamcat = false;
                $a = new stdClass;
                $a->nquestions = $gradeinfo->nquestions;
                $a->minquestions = $this->properties->minquestions;
                $this->add_message(get_string('numberofpagesviewednotice', 'amcat', $a));
        ...
        if (!$canmanage) {
            if ($data->gradeamcat) {
                    ...
                    $duration = $DB->get_field_sql(
                        "SELECT SUM(amcattime - starttime)
                                      ...
                                      WHERE amcatid = :amcatid
                                        AND userid = :userid",
                        array('userid' => $USER->id, 'amcatid' => $this->properties->id));
                    if (!$duration) {
                        $duration = 0;
                    }

                    // If student has not spend enough time in the amcat, display a message.
                ...
                if ($gradeinfo->attempts) {
                    ...
                    $grade = new stdClass();
                    $grade->amcatid = $this->properties->id;
                    ...
                    if (isset($USER->modattempts[$this->properties->id])) { // If reviewing, make sure update old grade record.
                        if (!$grades = $DB->get_records("amcat_grades",
                            array("amcatid" => $this->properties->id, "userid" => $USER->id), "completed DESC", '*', 0, 1)) {
                            throw new moodle_exception('cannotfindgrade', 'amcat');
                        }
                        $oldgrade = array_shift($grades);
                        $grade->id = $oldgrade->id;
                        $DB->update_record("amcat_grades", $grade);
                    } else {
                        $newgradeid = $DB->insert_record("amcat_grades", $grade);
                    }
                } else {
                    if ($this->properties->timelimit) {
                        if ($outoftime == 'normal') {
                ...
                amcat_update_grades($this, $USER->id);
        ...
        if ($this->properties->modattempts && !$canmanage) {
            ...
            if (!$attempts = $this->get_attempts($ntries)) {
                $attempts = array();
                $url = new moodle_url('/mod/amcat/view.php', array('id' => $cm->id));
            } else {
                ...
                // This will help to make sure that the student can leave the amcat via pushing the continue button.
                $lastattempt = end($attempts);
                $USER->modattempts[$this->properties->id] = $lastattempt->pageid;

                $url = new moodle_url('/mod/amcat/view.php', array('id' => $cm->id, 'pageid' => $pageid));
            }
            $data->reviewamcat = $url->out(false);
...
/**
 * Abstract class to provide a core functions to the all amcat classes
 *
 * This class should be abstracted by ALL classes with the amcat module to ensure
...
/**
 * Abstract class representation of a page associated with a amcat.
 *
 ...
 *
 * Database properties
 * @property int $id The id of this amcat page
 * @property int $amcatid The id of the amcat this page belongs to
 ...
 * @property-read amcat $amcat The amcat this page belongs to
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
abstract class amcat_page extends amcat_base {

    /**
     * A reference to the amcat this page belongs to
     * @var amcat
     */
    protected $amcat = null;
    /**
     * Contains the answers to this amcat_page once loaded
     * @var null|array
     */
    ...
    /**
     * This method gets called to display the page to the user taking the amcat
     * @abstract
    ...
    /**
     * Creates a new amcat_page within the database and returns the correct pagetype
     * object to use to interact with the new amcat
     *
     * @final
     * @static
     * @param object $properties
     * @param amcat $amcat
     * @return amcat_page Specialised object that extends amcat_page
     */
    final public static function create($properties, amcat $amcat, $context, $maxbytes) {
        global $DB;
        ...
        if ($properties->pageid) {
            $prevpage = $DB->get_record("amcat_pages", array("id" => $properties->pageid), 'id, nextpageid');
            if (!$prevpage) {
                print_error('cannotfindpages', 'amcat');
            }
            ...
            $nextpage = $DB->get_record('amcat_pages', array('amcatid'=>$amcat->id, 'prevpageid'=>0), 'id');
            if ($nextpage) {
        ...
        $newpage->id = $DB->insert_record("amcat_pages", $newpage);

        $editor = new stdClass;
        $editor->id = $newpage->id;
        $editor->contents_editor = $properties->contents_editor;
        $editor = file_postupdate_standard_editor($editor, 'contents', array('noclean'=>true, 'maxfiles'=>EDITOR_UNLIMITED_FILES, 'maxbytes'=>$maxbytes), $context, 'mod_amcat', 'page_contents', $editor->id);
        $DB->update_record("amcat_pages", $editor);

        if ($newpage->prevpageid > 0) {
            $DB->set_field("amcat_pages", "nextpageid", $newpage->id, array("id" => $newpage->prevpageid));
        }
        if ($newpage->nextpageid > 0) {
            $DB->set_field("amcat_pages", "prevpageid", $newpage->id, array("id" => $newpage->nextpageid));
        }

        $page = amcat_page::load($newpage, $amcat);
        ...
        $event = \mod_amcat\event\page_created::create($eventparams);
        $snapshot = clone($newpage);
        $snapshot->timemodified = 0;
        $event->add_record_snapshot('amcat_pages', $snapshot);
        $event->trigger();

        $amcat->add_message(get_string('insertedpage', 'amcat').': '.format_string($newpage->title, true), 'notifysuccess');
     ...
     * specialised object that extends amcat_page
     *
     * @final
     * @static
     * @param int $id
     * @param amcat $amcat
     * @return amcat_page Specialised amcat_page object
     */
    final public static function load($id, amcat $amcat) {
        global $DB;

        if (is_object($id) && !empty($id->qtype)) {
            $page = $id;
        } else {
            $page = $DB->get_record("amcat_pages", array("id" => $id));
            if (!$page) {
                print_error('cannotfindpages', 'amcat');
            }
        }
        ...
        $class = 'amcat_page_type_'.$manager->get_page_type_idstring($page->qtype);
        if (!class_exists($class)) {
            $class = 'amcat_page';
        }

        return new $class($page, $amcat);
    }

    /**
     * Deletes a amcat_page from the database as well as any associated records.
     * @final
     * @return bool
     */
    final public function delete() {
        ...
        $cm = get_coursemodule_from_instance('amcat', $this->amcat->id, $this->amcat->course);
        $context = context_module::instance($cm->id);

        // Delete files associated with attempts.
        $fs = get_file_storage();
        if ($attempts = $DB->get_records('amcat_attempts', array("pageid" => $this->properties->id))) {
            foreach ($attempts as $attempt) {
                $fs->delete_area_files($context->id, 'mod_amcat', 'essay_responses', $attempt->id);
                $fs->delete_area_files($context->id, 'mod_amcat', 'essay_answers', $attempt->id);
            }
        }

        // Then delete all the associated records...
        $DB->delete_records("amcat_attempts", array("pageid" => $this->properties->id));

        $DB->delete_records("amcat_branch", array("pageid" => $this->properties->id));

        // Delete files related to answers and responses.
        if ($answers = $DB->get_records("amcat_answers", array("pageid" => $this->properties->id))) {
            foreach ($answers as $answer) {
                $fs->delete_area_files($context->id, 'mod_amcat', 'page_answers', $answer->id);
                $fs->delete_area_files($context->id, 'mod_amcat', 'page_responses', $answer->id);
            }
        }

        // ...now delete the answers...
        $DB->delete_records("amcat_answers", array("pageid" => $this->properties->id));
        // ..and the page itself
        $DB->delete_records("amcat_pages", array("id" => $this->properties->id));
        ...
        $event = \mod_amcat\event\page_deleted::create($eventparams);
        $event->add_record_snapshot('amcat_pages', $this->properties);
        $event->trigger();

        // Delete files associated with this page.
        $fs->delete_area_files($context->id, 'mod_amcat', 'page_contents', $this->properties->id);
        ...
        if (!$this->properties->prevpageid && !$this->properties->nextpageid) {
            ...
            $page = $this->amcat->load_page($this->properties->nextpageid);
            $page->move(null, 0);
        } elseif (!$this->properties->nextpageid) {
            // this is the last page...
            $page = $this->amcat->load_page($this->properties->prevpageid);
            $page->move(0);
        } else {
            // page is in the middle...
            $prevpage = $this->amcat->load_page($this->properties->prevpageid);
            $nextpage = $this->amcat->load_page($this->properties->nextpageid);
    ...
    final public function move($nextpageid=null, $prevpageid=null) {
        ...
        $DB->update_record('amcat_pages', $obj);
    }
    ...
    final public function get_answers() {
        ...
        if ($this->answers === null) {
            $this->answers = array();
            $answers = $DB->get_records('amcat_answers', array('pageid'=>$this->properties->id, 'amcatid'=>$this->amcat->id), 'id');
            if (!$answers) {
                // It is possible that a amcat upgraded from Moodle 1.9 still
                // contains questions without any answers [MDL-25632].
                // debugging(get_string('cannotfindanswer', 'amcat'));
                return array();
            }
            foreach ($answers as $answer) {
                $this->answers[count($this->answers)] = new amcat_page_answer($answer);
            }
        }
        return $this->answers;
    }
    ...
    /**
     * Returns the amcat this page is associated with
     * @final
     * @return amcat
     */
    final protected function get_amcat() {
        return $this->amcat;
    }
    ...
    final public function record_attempt($context) {
        ...
        if ($result->noanswer) {
            ...
            $result->feedback  = get_string('noanswer', 'amcat');
        } else {
            if (!has_capability('mod/amcat:manage', $context)) {
                $nretakes = $DB->count_records("amcat_grades", array("amcatid"=>$this->amcat->id, "userid"=>$USER->id));

                // Get the number of attempts that have been made on this question for this student and retake,
                $nattempts = $DB->count_records('amcat_attempts', array('amcatid' => $this->amcat->id,
                ...
                if ($nattempts >= $this->amcat->maxattempts) {
                    $result->maxattemptsreached = true;
                    $result->feedback = get_string('maximumnumberofattemptsreached', 'amcat');
                    $result->newpageid = $this->amcat->get_next_page($this->properties->nextpageid);
                    return $result;
                }

                // record student's attempt
                $attempt = new stdClass;
                $attempt->amcatid = $this->amcat->id;
                ...
                if (isset($USER->modattempts[$this->amcat->id])) {
                    $attempt->retry = $nretakes - 1; // they are going through on review, $nretakes will be too high
                    $userisreviewing = true;
                }

                // Only insert a record if we are not reviewing the amcat.
                if (!$userisreviewing) {
                    if ($this->amcat->retake || (!$this->amcat->retake && $nretakes == 0)) {
                        $attempt->id = $DB->insert_record("amcat_attempts", $attempt);

                        list($updatedattempt, $updatedresult) = $this->on_after_write_attempt($attempt, $result);
                        if ($updatedattempt) {
                            $attempt = $updatedattempt;
                            $result = $updatedresult;
                            $DB->update_record("amcat_attempts", $attempt);
                        }
                        ...
                        $event = \mod_amcat\event\question_answered::create($eventparams);
                        $event->add_record_snapshot('amcat_attempts', $attempt);
                        $event->trigger();
                    ...
                    // When reviewing the amcat, the existing attemptid is also needed for the filearea options.
                    $params = [
                        'amcatid' => $attempt->amcatid,
                        'pageid' => $attempt->pageid,
                        'userid' => $attempt->userid,
                        'answerid' => $attempt->answerid,
                        'retry' => $attempt->retry
                    ];
                    $attempt->id = $DB->get_field('amcat_attempts', 'id', $params);
                }
                // "number of attempts remaining" message if $this->amcat->maxattempts > 1
                ...
                if (!$result->correctanswer && ($result->newpageid == 0)) {
                    ...
                    if ($nattempts >= $this->amcat->maxattempts) {
                        if ($this->amcat->maxattempts > 1) { // don't bother with message if only one attempt
                            $result->maxattemptsreached = true;
                        }
                        $result->newpageid = amcat_NEXTPAGE;
                    } else if ($this->amcat->maxattempts > 1) { // don't bother with message if only one attempt
                        $result->attemptsremaining = $this->amcat->maxattempts - $nattempts;
                    }
                }
            }
            ...
            if (empty($result->response)) {
                if (!$this->amcat->feedback && !$result->noanswer && !($this->amcat->review & !$result->correctanswer && !$result->isessayquestion)) {
                    // These conditions have been met:
                    //  1. The amcat manager has not supplied feedback to the student
                    ...
                    $result->response = get_string('defaultessayresponse', 'amcat');
                } else if ($result->correctanswer) {
                    $result->response = get_string('thatsthecorrectanswer', 'amcat');
                } else {
                    $result->response = get_string('thatsthewronganswer', 'amcat');
                }
            }

            if ($result->response) {
                if ($this->amcat->review && !$result->correctanswer && !$result->isessayquestion) {
                    $nretakes = $DB->count_records("amcat_grades", array("amcatid"=>$this->amcat->id, "userid"=>$USER->id));
                    $qattempts = $DB->count_records("amcat_attempts", array("userid"=>$USER->id, "retry"=>$nretakes, "pageid"=>$this->properties->id));
                    if ($qattempts == 1) {
                        $result->feedback = $OUTPUT->box(get_string("firstwrong", "amcat"), 'feedback');
                    } else {
                        if (!$result->maxattemptsreached) {
                            $result->feedback = $OUTPUT->box(get_string("secondpluswrong", "amcat"), 'feedback');
                        } else {
                            $result->feedback = $OUTPUT->box(get_string("finalwrong", "amcat"), 'feedback');
                        }
                    }
                } else {
                    $result->feedback = '';
                ...
                $result->feedback .= '<div class="correctanswer generalbox"><em>'
                        . get_string("youranswer", "amcat").'</em> : <div class="studentanswer mt-2 mb-2">';
                ...
                if ($this->properties->qoption) {
                    ...
                    foreach ($studentanswerresponse as $answer => $response) {
                        ...
                        if (!empty(trim($response))) {
                            ...
                            $studentresponsecontent = html_writer::div('<em>' . get_string("response", "amcat") .
    ...
    private function format_response($response, $context, $responseformat, $answerid, $options) {
        ...
        $convertstudentresponse = file_rewrite_pluginfile_urls($response, 'pluginfile.php',
            $context->id, 'mod_amcat', 'page_responses', $answerid);
    ...
    final protected function get_jump_name($jumpto) {
        ...
        if (!array_key_exists($jumpto, $jumpnames)) {
            if ($jumpto == amcat_THISPAGE) {
                $jumptitle = get_string('thispage', 'amcat');
            } elseif ($jumpto == amcat_NEXTPAGE) {
                $jumptitle = get_string('nextpage', 'amcat');
            } elseif ($jumpto == amcat_EOL) {
                $jumptitle = get_string('endofamcat', 'amcat');
            } elseif ($jumpto == amcat_UNSEENBRANCHPAGE) {
                $jumptitle = get_string('unseenpageinbranch', 'amcat');
            } elseif ($jumpto == amcat_PREVIOUSPAGE) {
                $jumptitle = get_string('previouspage', 'amcat');
            } elseif ($jumpto == amcat_RANDOMPAGE) {
                $jumptitle = get_string('randompageinbranch', 'amcat');
            } elseif ($jumpto == amcat_RANDOMBRANCH) {
                $jumptitle = get_string('randombranch', 'amcat');
            } elseif ($jumpto == amcat_CLUSTERJUMP) {
                $jumptitle = get_string('clusterjump', 'amcat');
            } else {
                if (!$jumptitle = $DB->get_field('amcat_pages', 'title', array('id' => $jumpto))) {
                    $jumptitle = '<strong>'.get_string('notdefined', 'amcat').'</strong>';
                }
            }
     ...
     * @param amcat $amcat
     */
    public function __construct($properties, amcat $amcat) {
        parent::__construct($properties);
        $this->amcat = $amcat;
    }
    ...
    public function save_answers_files($context, $maxbytes, &$answer, $answereditor = '', $responseeditor = '') {
        ...
        if (isset($answereditor['itemid'])) {
            $answer->answer = file_save_draft_area_files($answereditor['itemid'],
                    $context->id, 'mod_amcat', 'page_answers', $answer->id,
                    array('noclean' => true, 'maxfiles' => EDITOR_UNLIMITED_FILES, 'maxbytes' => $maxbytes),
                    $answer->answer, null);
            $DB->set_field('amcat_answers', 'answer', $answer->answer, array('id' => $answer->id));
        }
        if (isset($responseeditor['itemid'])) {
            $answer->response = file_save_draft_area_files($responseeditor['itemid'],
                    $context->id, 'mod_amcat', 'page_responses', $answer->id,
                    array('noclean' => true, 'maxfiles' => EDITOR_UNLIMITED_FILES, 'maxbytes' => $maxbytes),
                    $answer->response, null);
            $DB->set_field('amcat_answers', 'response', $answer->response, array('id' => $answer->id));
        }
    }
    ...
    public static function rewrite_answers_urls($answer, $rewriteanswer = true) {
        ...
        if ($rewriteanswer) {
            $answer->answer = file_rewrite_pluginfile_urls($answer->answer, 'pluginfile.php', $context->id,
                    'mod_amcat', 'page_answers', $answer->id);
        }
        $answer->response = file_rewrite_pluginfile_urls($answer->response, 'pluginfile.php', $context->id,
                'mod_amcat', 'page_responses', $answer->id);

        return $answer;
    }

    /**
     * Updates a amcat page and its answers within the database
     *
     * @param object $properties
     * @return bool
     */
    public function update($properties, $context = null, $maxbytes = null) {
        ...
        $properties = file_postupdate_standard_editor($properties, 'contents', array('noclean'=>true, 'maxfiles'=>EDITOR_UNLIMITED_FILES, 'maxbytes'=>$maxbytes), $context, 'mod_amcat', 'page_contents', $properties->id);
        ...
        if ($this->type == self::TYPE_STRUCTURE && $this->get_typeid() != amcat_PAGE_BRANCHTABLE) {
            // These page types have only one answer to save the jump and score.
            if (count($answers) > 1) {
                $answer = array_shift($answers);
                foreach ($answers as $a) {
                    $DB->delete_records('amcat_answers', array('id' => $a->id));
                }
            } else if (count($answers) == 1) {
                $answer = array_shift($answers);
            } else {
                $answer = new stdClass;
                $answer->amcatid = $properties->amcatid;
            ...
            if (!empty($answer->id)) {
                $DB->update_record("amcat_answers", $answer->properties());
            } else {
                $DB->insert_record("amcat_answers", $answer);
            }
        } else {
    ...
    public function is_unseen($param) {
        ...
        if (is_array($param)) {
            ...
            $nretakes = $param;
            if (!$DB->count_records("amcat_attempts", array("pageid"=>$this->properties->id, "userid"=>$USER->id, "retry"=>$nretakes))) {
                return true;
            }
        }
        return false;
    ...
    public function is_unanswered($nretakes) {
        global $DB, $USER;
        if (!$DB->count_records("amcat_attempts", array('pageid'=>$this->properties->id, 'userid'=>$USER->id, 'correct'=>1, 'retry'=>$nretakes))) {
            return true;
        }
        return false;
    }
    ...
    /**
     * Creates answers within the database for this amcat_page. Usually only ever
    ...
    public function create_answers($properties) {
        ...
        $newanswer->amcatid = $this->amcat->id;
        ...
        for ($i = 0; $i < ($this->amcat->maxanswers + 1); $i++) {
            ...
            if (isset($answer->answer) && $answer->answer != '') {
                ...
                $answer->id = $DB->insert_record("amcat_answers", $answer);
                if (isset($properties->response_editor[$i])) {
                ...
                $answers[$answer->id] = new amcat_page_answer($answer);
            }
        }
     ...
     * of answers permitted by the amcat.
     *
     * @param int $default
     * @return int
     */
    ...
    /**
     * Returns the properties of this amcat page as an object
     * @return stdClass;
     */
    public function properties() {
     ...
     * @return array
     */
    public static function get_jumptooptions($pageid, amcat $amcat) {
        global $DB;
        $jump = array();
        $jump[0] = get_string("thispage", "amcat");
        $jump[amcat_NEXTPAGE] = get_string("nextpage", "amcat");
        $jump[amcat_PREVIOUSPAGE] = get_string("previouspage", "amcat");
        $jump[amcat_EOL] = get_string("endofamcat", "amcat");
        ...
        $pages = $amcat->load_all_pages();
        if ($pages[$pageid]->qtype == amcat_PAGE_BRANCHTABLE || $amcat->is_sub_page_of_type($pageid, array(amcat_PAGE_BRANCHTABLE), array(amcat_PAGE_ENDOFBRANCH, amcat_PAGE_CLUSTER))) {
            $jump[amcat_UNSEENBRANCHPAGE] = get_string("unseenpageinbranch", "amcat");
            $jump[amcat_RANDOMPAGE] = get_string("randompageinbranch", "amcat");
        }
        if($pages[$pageid]->qtype == amcat_PAGE_CLUSTER || $amcat->is_sub_page_of_type($pageid, array(amcat_PAGE_CLUSTER), array(amcat_PAGE_ENDOFCLUSTER))) {
            $jump[amcat_CLUSTERJUMP] = get_string("clusterjump", "amcat");
        }
        if (!optional_param('firstpage', 0, PARAM_INT)) {
            $apageid = $DB->get_field("amcat_pages", "id", array("amcatid" => $amcat->id, "prevpageid" => 0));
            while (true) {
                if ($apageid) {
                    $title = $DB->get_field("amcat_pages", "title", array("id" => $apageid));
                    $jump[$apageid] = strip_tags(format_string($title,true));
                    $apageid = $DB->get_field("amcat_pages", "nextpageid", array("id" => $apageid));
                } else {
                    // last page reached
                    break;
                }
    ...
    public function get_contents() {
        ...
        if (!empty($this->properties->contents)) {
            ...
            $context = context_module::instance($PAGE->cm->id);
            $contents = file_rewrite_pluginfile_urls($this->properties->contents, 'pluginfile.php', $context->id, 'mod_amcat',
    ...
    public function display_answers(html_table $table) {
        ...
        foreach ($answers as $answer) {
            $cells = array();
            $cells[] = '<label>' . get_string('jump', 'amcat') . ' ' . $i . '</label>:';
    ...
    public function report_answers($answerpage, $answerdata, $useranswer, $pagestats, &$i, &$n) {
        ...
        foreach ($answers as $answer) {
            $data = get_string('jumpsto', 'amcat', $this->get_jump_name($answer->jumpto));
    ...
    public function get_jumps() {
        global $DB;
        $jumps = array();
        $params = array ("amcatid" => $this->amcat->id, "pageid" => $this->properties->id);
    ...
    public function get_files($includedirs = true, $updatedsince = 0) {
        $fs = get_file_storage();
        return $fs->get_area_files($this->amcat->context->id, 'mod_amcat', 'page_contents', $this->properties->id,
 ...
 * @property int $amcatid The ID of the amcat this answer belongs to
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class amcat_page_answer extends amcat_base {

    /**
     * Loads an page answer from the DB
     *
     * @param int $id
     * @return amcat_page_answer
     */
    public static function load($id) {
        global $DB;
        $answer = $DB->get_record("amcat_answers", array("id" => $id));
        return new amcat_page_answer($answer);
    }
     ...
     * @return array
     */
    public static function create($properties, amcat_page $page) {
    ...
    public function get_files($includedirs = true, $updatedsince = 0) {
        ...
        $amcat = amcat::load($this->properties->amcatid);
        $fs = get_file_storage();
        $answerfiles = $fs->get_area_files($amcat->context->id, 'mod_amcat', 'page_answers', $this->properties->id,
                                            'itemid, filepath, filename', $includedirs, $updatedsince);
        $responsefiles = $fs->get_area_files($amcat->context->id, 'mod_amcat', 'page_responses', $this->properties->id,
                                            'itemid, filepath, filename', $includedirs, $updatedsince);
 ...
 * different page types located in mod/amcat/pagetypes.
 *
 * @copyright  2009 Sam Hemelryk
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class amcat_page_type_manager {
    ...
    /**
     * Retrieves the amcat page type manager object
     *
     * If the object hasn't yet been created it is created here.
     *
     * @staticvar amcat_page_type_manager $pagetypemanager
     * @param amcat $amcat
     * @return amcat_page_type_manager
     */
    public static function get(amcat $amcat) {
        static $pagetypemanager;
        if (!($pagetypemanager instanceof amcat_page_type_manager)) {
            $pagetypemanager = new amcat_page_type_manager();
            $pagetypemanager->load_amcat_types($amcat);
        }
        return $pagetypemanager;
    }

    /**
     * Finds and loads all amcat page types in mod/amcat/pagetypes
     *
     * @param amcat $amcat
     */
    public function load_amcat_types(amcat $amcat) {
        global $CFG;
        $basedir = $CFG->dirroot.'/mod/amcat/pagetypes/';
        $dir = dir($basedir);
        while (false !== ($entry = $dir->read())) {
            ...
            $class = 'amcat_page_type_'.strtok($entry,'.');
            if (class_exists($class)) {
                $pagetype = new $class(new stdClass, $amcat);
    ...
    /**
     * Loads a page for the provided amcat given it's id
     *
     * This function loads a page from the amcat when given both the amcat it belongs
     * to as well as the page's id.
     * If the page doesn't exist an error is thrown
     *
     * @param int $pageid The id of the page to load
     * @param amcat $amcat The amcat the page belongs to
     * @return amcat_page A class that extends amcat_page
     */
    public function load_page($pageid, amcat $amcat) {
        global $DB;
        if (!($page =$DB->get_record('amcat_pages', array('id'=>$pageid, 'amcatid'=>$amcat->id)))) {
            print_error('cannotfindpages', 'amcat');
        }
        $pagetype = get_class($this->types[$page->qtype]);
        $page = new $pagetype($page, $amcat);
        return $page;
    }
    ...
    protected function check_page_order($page1, $page2) {
        ...
        if (empty($page1)) {
            if ($page2->prevpageid != 0) {
                ...
                $page2->prevpageid = 0;
                $DB->set_field("amcat_pages", "prevpageid", 0, array("id" => $page2->id));
            }
        } else if (empty($page2)) {
            if ($page1->nextpageid != 0) {
                debugging("***nextpageid of page " . $page1->id . " set to 0***");
                $page1->nextpageid = 0;
                $DB->set_field("amcat_pages", "nextpageid", 0, array("id" => $page1->id));
            }
        } else {
            if ($page1->nextpageid != $page2->id) {
                debugging("***nextpageid of page " . $page1->id . " set to " . $page2->id . "***");
                $page1->nextpageid = $page2->id;
                $DB->set_field("amcat_pages", "nextpageid", $page2->id, array("id" => $page1->id));
            }
            if ($page2->prevpageid != $page1->id) {
                debugging("***prevpageid of page " . $page2->id . " set to " . $page1->id . "***");
                $page2->prevpageid = $page1->id;
                $DB->set_field("amcat_pages", "prevpageid", $page1->id, array("id" => $page2->id));
            }
        }
    }

    /**
     * This function loads ALL pages that belong to the amcat.
     *
     * @param amcat $amcat
     * @return array An array of amcat_page_type_*
     */
    public function load_all_pages(amcat $amcat) {
        global $DB;
        if (!($pages =$DB->get_records('amcat_pages', array('amcatid'=>$amcat->id)))) {
            return array(); // Records returned empty.
        }
        foreach ($pages as $key=>$page) {
            $pagetype = get_class($this->types[$page->qtype]);
            $pages[$key] = new $pagetype($page, $amcat);
        }
     ...
     * @return amcat_add_page_form_base
     */
    public function get_page_form($type, $arguments) {
        $class = 'amcat_add_page_form_'.$this->get_page_type_idstring($type);
        if (!class_exists($class) || get_parent_class($class)!=='amcat_add_page_form_base') {
            debugging('amcat page type unknown class requested '.$class, DEBUG_DEVELOPER);
            $class = 'amcat_add_page_form_selection';
        } else if ($class === 'amcat_add_page_form_unknown') {
            $class = 'amcat_add_page_form_selection';
        }
