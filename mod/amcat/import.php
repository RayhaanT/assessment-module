...
require_once($CFG->dirroot.'/mod/amcat/import_form.php');
require_once($CFG->dirroot.'/mod/amcat/format.php');  // Parent class

$id     = required_param('id', PARAM_INT);         // Course Module ID
$pageid = optional_param('pageid', '', PARAM_INT); // Page ID

$PAGE->set_url('/mod/amcat/import.php', array('id'=>$id, 'pageid'=>$pageid));
...
$strimportquestions = get_string("importquestions", "amcat");
...
$mform = new amcat_import_form(null, array('formats'=>amcat_get_import_export_formats('import')));
$mform->set_data($data);
    ...
    echo $OUTPUT->header();
    echo $OUTPUT->heading(format_string($amcat->name), 2);
    echo $OUTPUT->heading_with_help($strimportquestions, 'importquestions', 'amcat', '', '', 3);
...
if ($data = $mform->get_data()) {
    ...
    if (! $format->importpreprocess()) {
                print_error('preprocesserror', 'amcat');
            }

    // Process the uploaded file
    if (! $format->importprocess($importfile, $amcat, $pageid)) {
                print_error('processerror', 'amcat');
            }

    // In case anything needs to be done after
    if (! $format->importpostprocess()) {
                print_error('postprocesserror', 'amcat');
            }
