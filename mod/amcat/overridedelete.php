...
/**
 * This page handles deleting amcat overrides
 *
 * @package    mod_amcat
...
if (! $cm = get_coursemodule_from_instance("amcat", $amcat->id, $amcat->course)) {
...
$url = new moodle_url('/mod/amcat/overridedelete.php', array('id' => $override->id));
$confirmurl = new moodle_url($url, array('id' => $override->id, 'confirm' => 1));
$cancelurl = new moodle_url('/mod/amcat/overrides.php', array('cmid' => $cm->id));
...
if ($confirm) {
    ...
    $amcat->delete_override($override->id);

    redirect($cancelurl);
}

// Prepare the page to show the confirmation form.
$stroverride = get_string('override', 'amcat');
...
if ($override->groupid) {
    ...
    $confirmstr = get_string("overridedeletegroupsure", "amcat", $group->name);
} else {
    $namefields = get_all_user_name_fields(true);
    $user = $DB->get_record('user', array('id' => $override->userid),
            'id, ' . $namefields);
    $confirmstr = get_string("overridedeleteusersure", "amcat", fullname($user));
}
