...
require_once($CFG->dirroot . '/mod/amcat/pagetypes/essay.php');
require_once($CFG->dirroot . '/mod/amcat/pagetypes/matching.php');
require_once($CFG->dirroot . '/mod/amcat/pagetypes/multichoice.php');
...
class provider implements
    ...
    public static function get_metadata(collection $collection) : collection {
        $collection->add_database_table('amcat_attempts', [
        ...
        $collection->add_database_table('amcat_grades', [
        ...
        $collection->add_database_table('amcat_timer', [
            'userid' => 'privacy:metadata:timer:userid',
            'starttime' => 'privacy:metadata:timer:starttime',
            'amcattime' => 'privacy:metadata:timer:amcattime',
        ...
        $collection->add_database_table('amcat_branch', [
        ...
        $collection->add_database_table('amcat_overrides', [
        ...
        $collection->add_user_preference('amcat_view', 'privacy:metadata:userpref:amcatview');
    ...
    public static function get_contexts_for_userid(int $userid) : \core_privacy\local\request\contextlist {
        ...
        $sql = "
         ...
         LEFT JOIN {amcat_attempts} la
                ON la.amcatid = l.id
               AND la.userid = :userid1
         LEFT JOIN {amcat_branch} lb
                ON lb.amcatid = l.id
               AND lb.userid = :userid2
         LEFT JOIN {amcat_grades} lg
                ON lg.amcatid = l.id
               AND lg.userid = :userid3
         LEFT JOIN {amcat_overrides} lo
                ON lo.amcatid = l.id
               AND lo.userid = :userid4
         LEFT JOIN {amcat_timer} lt
                ON lt.amcatid = l.id
    ...
    public static function get_users_in_context(userlist $userlist) {
        ...
        // Mapping of amcat tables which may contain user data.
        $joins = [
            'amcat_attempts',
            'amcat_branch',
            'amcat_grades',
    ...
    public static function export_user_data(approved_contextlist $contextlist) {
        ...
        // If the context export was requested, then let's at least describe the amcat.
        ...
        // Find the amcat IDs.
        $amcatidstocmids = static::get_amcat_ids_to_cmids_from_cmids($cmids);

        // Prepare the common SQL fragments.
        list($inamcatsql, $inamcatparams) = $DB->get_in_or_equal(array_keys($amcatidstocmids), SQL_PARAMS_NAMED);
        $sqluseramcat = "userid = :userid AND amcatid $inamcatsql";
        $paramsuseramcat = array_merge($inamcatparams, ['userid' => $userid]);

        // Export the overrides.
        $recordset = $DB->get_recordset_select('amcat_overrides', $sqluseramcat, $paramsuseramcat);
        static::recordset_loop_and_export($recordset, 'amcatid', null, function($carry, $record) {
            // We know that there is only one row per amcat, so no need to use $carry.
            return (object) [
        ...
        }, function($amcatid, $data) use ($amcatidstocmids) {
            $context = context_module::instance($amcatidstocmids[$amcatid]);
            writer::with_context($context)->export_related_data([], 'overrides', $data);
        });

        // Export the grades.
        $recordset = $DB->get_recordset_select('amcat_grades', $sqluseramcat, $paramsuseramcat, 'amcatid, completed');
        static::recordset_loop_and_export($recordset, 'amcatid', [], function($carry, $record) {
            $carry[] = (object) [
                'grade' => $record->grade,
        ...
        // Export the timers.
        $recordset = $DB->get_recordset_select('amcat_timer', $sqluseramcat, $paramsuseramcat, 'amcatid, starttime');
        static::recordset_loop_and_export($recordset, 'amcatid', [], function($carry, $record) {
            $carry[] = (object) [
                'starttime' => transform::datetime($record->starttime),
                'lastactivity' => transform::datetime($record->amcattime),
        ...
        $sql = "
         ...
         LEFT JOIN {amcat_branch} lb
                ON lb.pageid = lp.id
               AND lb.userid = :userid2
         LEFT JOIN {amcat_pages} lpb
                ON lpb.id = lb.nextpageid
             WHERE lp.amcatid $inamcatsql
               AND (la.id IS NOT NULL OR lb.id IS NOT NULL)
          ORDER BY lp.amcatid, lp.id, la.retry, lb.retry, la.id, lb.id";
        $params = array_merge($inamcatparams, ['userid1' => $userid, 'userid2' => $userid]);

        $recordset = $DB->get_recordset_sql($sql, $params);
        static::recordset_loop_and_export($recordset, 'amcatid', [], function($carry, $record) use ($amcatidstocmids) {
            $context = context_module::instance($amcatidstocmids[$record->amcatid]);
            ...
            $pagefilespath = [get_string('privacy:path:pages', 'mod_amcat'), $record->page_id];
            writer::with_context($context)->export_area_files($pagefilespath, 'mod_amcat', 'page_contents', $record->page_id);
            ...
            if (isset($record->attempt_id)) {
                ...
                if (!empty($record->nextpage_id)) {
                    ...
                    $wentto = get_string('endofamcat', 'mod_amcat');
                }
    ...
    public static function export_user_preferences(int $userid) {
        $amcatview = get_user_preferences('amcat_view', null, $userid);
        if ($amcatview !== null) {
            $value = $amcatview;
            ...
            if ($amcatview == 'full') {
                $value = get_string('full', 'mod_amcat');
            } else if ($amcatview == 'collapsed') {
                $value = get_string('collapsed', 'mod_amcat');
            }

            writer::export_user_preference('mod_amcat', 'amcat_view', $amcatview,
                get_string('privacy:metadata:userpref:amcatview', 'mod_amcat'));
        }
    }
    ...
    public static function delete_data_for_all_users_in_context(context $context) {
        ...
        if (!$amcatid = static::get_amcat_id_from_context($context)) {
            return;
        }

        $DB->delete_records('amcat_attempts', ['amcatid' => $amcatid]);
        $DB->delete_records('amcat_branch', ['amcatid' => $amcatid]);
        $DB->delete_records('amcat_grades', ['amcatid' => $amcatid]);
        $DB->delete_records('amcat_timer', ['amcatid' => $amcatid]);
        $DB->delete_records_select('amcat_overrides', 'amcatid = :id AND userid IS NOT NULL', ['id' => $amcatid]);
    ...
    public static function delete_data_for_user(approved_contextlist $contextlist) {
        ...
        // Find the amcat IDs.
        $amcatidstocmids = static::get_amcat_ids_to_cmids_from_cmids($cmids);
        $amcatids = array_keys($amcatidstocmids);
        if (empty($amcatids)) {
            return;
        }

        // Prepare the SQL we'll need below.
        list($insql, $inparams) = $DB->get_in_or_equal($amcatids, SQL_PARAMS_NAMED);
        $sql = "amcatid $insql AND userid = :userid";
        $params = array_merge($inparams, ['userid' => $userid]);

        // Delete the attempt files.
        $fs = get_file_storage();
        $recordset = $DB->get_recordset_select('amcat_attempts', $sql, $params, '', 'id, amcatid');
        foreach ($recordset as $record) {
            $cmid = $amcatidstocmids[$record->amcatid];
            $context = context_module::instance($cmid);
            $fs->delete_area_files($context->id, 'mod_amcat', 'essay_responses', $record->id);
            $fs->delete_area_files($context->id, 'mod_amcat', 'essay_answers', $record->id);
        }
        $recordset->close();

        // Delete all the things.
        $DB->delete_records_select('amcat_attempts', $sql, $params);
        $DB->delete_records_select('amcat_branch', $sql, $params);
        $DB->delete_records_select('amcat_grades', $sql, $params);
        $DB->delete_records_select('amcat_timer', $sql, $params);
        $DB->delete_records_select('amcat_overrides', $sql, $params);
    }
    ...
    public static function delete_data_for_users(approved_userlist $userlist) {
        ...
        $context = $userlist->get_context();
        $amcatid = static::get_amcat_id_from_context($context);
        ...
        $sql = "amcatid = :amcatid AND userid {$insql}";
        $params = array_merge($inparams, ['amcatid' => $amcatid]);
     ...
     * @return int
     */
    protected static function get_amcat_id_from_context(context_module $context) {
        $cm = get_coursemodule_from_id('amcat', $context->instanceid);
        return $cm ? (int) $cm->instance : 0;
    }

    /**
     * Return a dict of amcat IDs mapped to their course module ID.
     *
     * @param array $cmids The course module IDs.
     * @return array In the form of [$amcatid => $cmid].
     */
    protected static function get_amcat_ids_to_cmids_from_cmids(array $cmids) {
        global $DB;
        ...
        $sql = "
            SELECT l.id, cm.id AS cmid
              ...
              JOIN {course_modules} cm
               ...
               AND cm.module = m.id
             WHERE cm.id $insql";
        $params = array_merge($inparams, ['amcat' => 'amcat']);
    ...
    protected static function transform_attempt(stdClass $data, context_module $context) {
        ...
        if ($answer !== null) {
            if ($data->page_qtype == amcat_PAGE_ESSAY) {
                // Essay questions serialise data in the answer field.
                $info = \amcat_page_type_essay::extract_useranswer($answer);
                $answerfilespath = [get_string('privacy:path:essayanswers', 'mod_amcat'), $data->attempt_id];
                $answer = format_text(
                    writer::with_context($context)->rewrite_pluginfile_urls(
                        $answerfilespath,
                        'mod_amcat',
                ...
                writer::with_context($context)->export_area_files($answerfilespath, 'mod_amcat',
                ...
                if ($info->response !== null) {
                    ...
                    $responsefilespath = [get_string('privacy:path:essayresponses', 'mod_amcat'), $data->attempt_id];
                    ...
                    writer::with_context($context)->export_area_files($responsefilespath, 'mod_amcat',
                        'essay_responses', $data->page_id);

                }

            } else if ($data->page_qtype == amcat_PAGE_MULTICHOICE && $data->page_qoption) {
                ...
                $records = $DB->get_records_select('amcat_answers', "id $insql", $inparams, $orderby);
            ...
            } else if ($data->page_qtype == amcat_PAGE_MATCHING) {
                // Matching questions need sorting.
                $chosen = explode(',', $answer);
                $answers = $DB->get_records_select('amcat_answers', 'pageid = :pageid', ['pageid' => $data->page_id],
