 ...
 * @return bool
 */
function xmldb_amcat_upgrade($oldversion) {
    global $CFG, $DB;

    $dbman = $DB->get_manager();

    if ($oldversion < 2016120515) {
        // Define new fields to be added to amcat.
        $table = new xmldb_table('amcat');
        ...
        // amcat savepoint reached.
        upgrade_mod_savepoint(true, 2016120515, 'amcat');
    }
    if ($oldversion < 2016120516) {
        // New field for amcat_timer.
        $table = new xmldb_table('amcat_timer');
        ...
        // amcat savepoint reached.
        upgrade_mod_savepoint(true, 2016120516, 'amcat');
    }
    ...
    if ($oldversion < 2017051501) {
        ...
        // Delete orphaned amcat answer and response files.
        $sql = "SELECT DISTINCT f.contextid, f.component, f.filearea, f.itemid
                  FROM {files} f
             LEFT JOIN {amcat_answers} la ON f.itemid = la.id
                 WHERE component = :component
                   AND (filearea = :fileareaanswer OR filearea = :filearearesponse)
                   AND la.id IS NULL";

        $orphanedfiles = $DB->get_recordset_sql($sql, array('component' => 'mod_amcat', 'fileareaanswer' => 'page_answers',
        ...
        upgrade_mod_savepoint(true, 2017051501, 'amcat');
    }

    if ($oldversion < 2019062400) {
        // Delete orphaned group overrides.
        $DB->delete_records_select('amcat_overrides', 'groupid = 0 AND userid IS NULL');

        upgrade_mod_savepoint(true, 2019062400, 'amcat');
    }
