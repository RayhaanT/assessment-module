...
/**
 * This page handles listing of amcat overrides
 *
 * @package    mod_amcat
...
$amcat = $DB->get_record('amcat', array('id' => $cm->instance), '*', MUST_EXIST);
...
$amcatgroupmode = groups_get_activity_groupmode($cm);
$accessallgroups = ($amcatgroupmode == NOGROUPS) || has_capability('moodle/site:accessallgroups', $context);
...
$url = new moodle_url('/mod/amcat/overrides.php', array('cmid' => $cm->id, 'mode' => $mode));

$PAGE->set_url($url);

// Display a list of overrides.
$PAGE->set_pagelayout('admin');
$PAGE->set_title(get_string('overrides', 'amcat'));
...
$sql = 'SELECT o.id
     ...
     LEFT JOIN {groups} g ON o.groupid = g.id
         WHERE o.groupid IS NOT NULL
               AND g.id IS NULL
               AND o.amcatid = ?';
$params = array($amcat->id);
$orphaned = $DB->get_records_sql($sql, $params);
if (!empty($orphaned)) {
    $DB->delete_records_list('amcat_overrides', 'id', array_keys($orphaned));
}
...
if ($groupmode) {
    ...
    if ($groups) {
        ...
        $sql = "SELECT o.*, g.name
                 ...
                 WHERE o.amcatid = :amcatid AND g.id $insql
              ORDER BY g.name";
    ...
    if ($accessallgroups) {
        ...
        $sql = 'SELECT o.*, ' . get_all_user_name_fields(true, 'u') . '
                 ...
                 WHERE o.amcatid = :amcatid AND gm.groupid ' . $insql . '
              ORDER BY ' . $sort;
...
$overridedeleteurl = new moodle_url('/mod/amcat/overridedelete.php');
$overrideediturl = new moodle_url('/mod/amcat/overrideedit.php');
...
foreach ($overrides as $override) {
    ...
    if (isset($override->available)) {
        $fields[] = get_string('amcatopens', 'amcat');
        $values[] = $override->available > 0 ?
                userdate($override->available) : get_string('noopen', 'amcat');
    }

    // Format deadline.
    if (isset($override->deadline)) {
        $fields[] = get_string('amcatcloses', 'amcat');
        $values[] = $override->deadline > 0 ?
                userdate($override->deadline) : get_string('noclose', 'amcat');
    }

    // Format timelimit.
    if (isset($override->timelimit)) {
        $fields[] = get_string('timelimit', 'amcat');
        $values[] = $override->timelimit > 0 ?
                format_time($override->timelimit) : get_string('none', 'amcat');
    }

    // Format option to try a question again.
    if (isset($override->review)) {
        $fields[] = get_string('displayreview', 'amcat');
    ...
    if (isset($override->maxattempts)) {
        $fields[] = get_string('maximumnumberofattempts', 'amcat');
    ...
    if (isset($override->retake)) {
        $fields[] = get_string('retakesallowed', 'amcat');
    ...
    if (isset($override->password)) {
        $fields[] = get_string('usepassword', 'amcat');
        $values[] = $override->password !== '' ?
                get_string('enabled', 'amcat') : get_string('none', 'amcat');
    }
...
echo html_writer::start_tag('div', array('id' => 'amcatoverrides'));
if (count($table->data)) {
    echo html_writer::table($table);
}
if ($hasinactive) {
    echo $OUTPUT->notification(get_string('inactiveoverridehelp', 'amcat'), 'dimmed_text');
}
...
if ($groupmode) {
    if (empty($groups)) {
        // There are no groups.
        echo $OUTPUT->notification(get_string('groupsnone', 'amcat'), 'error');
        $options['disabled'] = true;
    }
    echo $OUTPUT->single_button($overrideediturl->out(true,
            array('action' => 'addgroup', 'cmid' => $cm->id)),
            get_string('addnewgroupoverride', 'amcat'), 'post', $options);
} else {
    $users = array();
    // See if there are any users in the amcat.
    if ($accessallgroups) {
        $users = get_enrolled_users($context, '', 0, 'u.id');
        $nousermessage = get_string('usersnone', 'amcat');
    } else if ($groups) {
        ...
        $nousermessage = get_string('groupsnone', 'amcat');
    }
    ...
    echo $OUTPUT->single_button($overrideediturl->out(true,
            ...
            get_string('addnewuseroverride', 'amcat'), 'get', $options);
}
