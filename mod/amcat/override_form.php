...
/**
 * Settings form for overrides in the amcat module.
 *
 * @package    mod_amcat
...
require_once($CFG->dirroot . '/mod/amcat/mod_form.php');
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class amcat_override_form extends moodleform {

    /** @var object course module object. */
    protected $cm;

    /** @var object the amcat settings object. */
    protected $amcat;

    /** @var context the amcat context. */
    protected $context;
     ...
     * @param object $amcat the amcat settings object.
     * @param object $context the amcat context.
     ...
     */
    public function __construct($submiturl, $cm, $amcat, $context, $groupmode, $override) {
    ...
    protected function definition() {
        ...
        $mform->addElement('header', 'override', get_string('override', 'amcat'));

        $amcatgroupmode = groups_get_activity_groupmode($cm);
        $accessallgroups = ($amcatgroupmode == NOGROUPS) || has_capability('moodle/site:accessallgroups', $this->context);
        ...
        if ($this->groupmode) {
            ...
            if ($this->groupid) {
                ...
                $mform->addElement('select', 'groupid',
                        get_string('overridegroup', 'amcat'), $groupchoices);
                ...
                if (empty($groups)) {
                    // Generate an error.
                    $link = new moodle_url('/mod/amcat/overrides.php', array('cmid' => $cm->id));
                    print_error('groupsnone', 'amcat', $link);
                }
            ...
            if ($this->userid) {
                ...
                $mform->addElement('select', 'userid',
                        get_string('overrideuser', 'amcat'), $userchoices);
                $mform->freeze('userid');
            } else {
                ...
                if (empty($users)) {
                    // Generate an error.
                    $link = new moodle_url('/mod/amcat/overrides.php', array('cmid' => $cm->id));
                    print_error('usersnone', 'amcat', $link);
                }
        ...
        $mform->addElement('passwordunmask', 'password', get_string('usepassword', 'amcat'));
        $mform->setType('password', PARAM_TEXT);
        $mform->addHelpButton('password', 'usepassword', 'amcat');
        $mform->setDefault('password', $this->amcat->password);;

        // Open and close dates.
        $mform->addElement('date_time_selector', 'available', get_string('available', 'amcat'), array('optional' => true));
        $mform->setDefault('available', $this->amcat->available);

        $mform->addElement('date_time_selector', 'deadline', get_string('deadline', 'amcat'), array('optional' => true));
        $mform->setDefault('deadline', $this->amcat->deadline);

        // amcat time limit.
        $mform->addElement('duration', 'timelimit',
                get_string('timelimit', 'amcat'), array('optional' => true));
        if ($this->amcat->timelimit != 0) {
            $mform->setDefault('timelimit', 0);
        } else {
            $mform->setDefault('timelimit', $this->amcat->timelimit);
        }
        ...
        $mform->setDefault('review', $this->amcat->review);
        ...
        $mform->setDefault('maxattempts', $this->amcat->maxattempts);

        // Retake allowed.
        $mform->addElement('selectyesno', 'retake', get_string('retakesallowed', 'amcat'));
        $mform->addHelpButton('retake', 'retakesallowed', 'amcat');
        $mform->setDefault('retake', $this->amcat->retake);

        // Submit buttons.
        $mform->addElement('submit', 'resetbutton',
                get_string('reverttodefaults', 'amcat'));

        $buttonarray = array();
        $buttonarray[] = $mform->createElement('submit', 'submitbutton',
                get_string('save', 'amcat'));
        $buttonarray[] = $mform->createElement('submit', 'againbutton',
                get_string('saveoverrideandstay', 'amcat'));
    ...
    public function validation($data, $files) {
        ...
        if (!$changed) {
            $errors['available'] = get_string('nooverridedata', 'amcat');
        }
