@mod @mod_amcat
Feature: In a amcat activity, students can see questions in random order and a single question drawn from a branch
  In order to create a amcat with a cluster and a subcluster
  As a teacher
  ...
  Scenario: amcat with subcluster
    Given I add a "amcat" to section "1" and I fill the form with:
      | Name | amcat with subcluster |
      | Description | Test amcat description |
    And I follow "amcat with subcluster"
    ...
    When I follow "amcat with subcluster"
