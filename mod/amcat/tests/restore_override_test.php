 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_amcat_restore_override_testcase extends restore_date_testcase {
    ...
    public function test_restore_overrides() {
        ...
        $amcatgen = $this->getDataGenerator()->get_plugin_generator('mod_amcat');
        $amcat = $amcatgen->create_instance(['course' => $course->id]);
        ...
        $DB->insert_record('amcat_overrides', $groupoverride1);
        ...
        $DB->insert_record('amcat_overrides', $groupoverride2);

        // Current quiz overrides.
        $overrides = $DB->get_records('amcat_overrides', ['amcatid' => $amcat->id]);
        ...
        $DB->insert_record('amcat_overrides', $useroverride);
        ...
        $newquiz = $DB->get_record('amcat', ['course' => $newcourseid]);
        $overrides = $DB->get_records('amcat_overrides', ['amcatid' => $newquiz->id]);
