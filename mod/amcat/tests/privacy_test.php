...
use mod_amcat\privacy\provider;
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_amcat_privacy_testcase extends provider_testcase {
    ...
    public function test_get_contexts_for_userid() {
        ...
        $cm1 = $dg->create_module('amcat', ['course' => $c1]);
        $cm2 = $dg->create_module('amcat', ['course' => $c1]);
        $cm3 = $dg->create_module('amcat', ['course' => $c1]);
    ...
    public function test_delete_data_for_all_users_in_context() {
        ...
        $assertcm1nochange = function() use ($DB, $u1, $u2, $cm1) {
            $this->assertTrue($DB->record_exists('amcat_attempts', ['userid' => $u1->id, 'amcatid' => $cm1->id]));
            $this->assertTrue($DB->record_exists('amcat_grades', ['userid' => $u1->id, 'amcatid' => $cm1->id]));
            $this->assertTrue($DB->record_exists('amcat_timer', ['userid' => $u1->id, 'amcatid' => $cm1->id]));
            $this->assertTrue($DB->record_exists('amcat_branch', ['userid' => $u1->id, 'amcatid' => $cm1->id]));
            $this->assertTrue($DB->record_exists('amcat_overrides', ['userid' => $u1->id, 'amcatid' => $cm1->id]));
            $this->assertTrue($DB->record_exists('amcat_attempts', ['userid' => $u2->id, 'amcatid' => $cm1->id]));
            $this->assertTrue($DB->record_exists('amcat_grades', ['userid' => $u2->id, 'amcatid' => $cm1->id]));
            $this->assertTrue($DB->record_exists('amcat_timer', ['userid' => $u2->id, 'amcatid' => $cm1->id]));
            $this->assertTrue($DB->record_exists('amcat_branch', ['userid' => $u2->id, 'amcatid' => $cm1->id]));
            $this->assertTrue($DB->record_exists('amcat_overrides', ['userid' => $u2->id, 'amcatid' => $cm1->id]));
        };
        $assertcm2nochange = function() use ($DB, $u1, $u2, $cm2) {
            $this->assertTrue($DB->record_exists('amcat_attempts', ['userid' => $u1->id, 'amcatid' => $cm2->id]));
            $this->assertTrue($DB->record_exists('amcat_grades', ['userid' => $u1->id, 'amcatid' => $cm2->id]));
            $this->assertTrue($DB->record_exists('amcat_timer', ['userid' => $u1->id, 'amcatid' => $cm2->id]));
            $this->assertTrue($DB->record_exists('amcat_branch', ['userid' => $u1->id, 'amcatid' => $cm2->id]));
            $this->assertTrue($DB->record_exists('amcat_overrides', ['userid' => $u1->id, 'amcatid' => $cm2->id]));
            $this->assertTrue($DB->record_exists('amcat_attempts', ['userid' => $u2->id, 'amcatid' => $cm2->id]));
            $this->assertTrue($DB->record_exists('amcat_grades', ['userid' => $u2->id, 'amcatid' => $cm2->id]));
            $this->assertTrue($DB->record_exists('amcat_timer', ['userid' => $u2->id, 'amcatid' => $cm2->id]));
            $this->assertTrue($DB->record_exists('amcat_branch', ['userid' => $u2->id, 'amcatid' => $cm2->id]));
            $this->assertTrue($DB->record_exists('amcat_overrides', ['userid' => $u2->id, 'amcatid' => $cm2->id]));
        };
        ...
        $assertcm2nochange();
        $this->assertFalse($DB->record_exists('amcat_attempts', ['userid' => $u1->id, 'amcatid' => $cm1->id]));
        $this->assertFalse($DB->record_exists('amcat_grades', ['userid' => $u1->id, 'amcatid' => $cm1->id]));
        $this->assertFalse($DB->record_exists('amcat_timer', ['userid' => $u1->id, 'amcatid' => $cm1->id]));
        $this->assertFalse($DB->record_exists('amcat_branch', ['userid' => $u1->id, 'amcatid' => $cm1->id]));
        $this->assertFalse($DB->record_exists('amcat_overrides', ['userid' => $u1->id, 'amcatid' => $cm1->id]));
        $this->assertFalse($DB->record_exists('amcat_attempts', ['userid' => $u2->id, 'amcatid' => $cm1->id]));
        $this->assertFalse($DB->record_exists('amcat_grades', ['userid' => $u2->id, 'amcatid' => $cm1->id]));
        $this->assertFalse($DB->record_exists('amcat_timer', ['userid' => $u2->id, 'amcatid' => $cm1->id]));
        $this->assertFalse($DB->record_exists('amcat_branch', ['userid' => $u2->id, 'amcatid' => $cm1->id]));
        $this->assertFalse($DB->record_exists('amcat_overrides', ['userid' => $u2->id, 'amcatid' => $cm1->id]));
    }
    ...
    public function test_delete_data_for_user() {
        ...
        // Delete the course: no change.
        provider::delete_data_for_user(new approved_contextlist($u1, 'mod_amcat', [context_course::instance($c1->id)->id]));
        $assertu1nochange();
        $assertu2nochange();

        // Delete another module: no change.
        provider::delete_data_for_user(new approved_contextlist($u1, 'mod_amcat', [context_module::instance($cm3->cmid)->id]));
        $assertu1nochange();
        $assertu2nochange();

        // Delete u1 in cm1: no change for u2 and in cm2.
        provider::delete_data_for_user(new approved_contextlist($u1, 'mod_amcat', [context_module::instance($cm1->cmid)->id]));
        $assertu2nochange();
        $this->assertFalse($DB->record_exists('amcat_attempts', ['userid' => $u1->id, 'amcatid' => $cm1->id]));
        $this->assertFalse($DB->record_exists('amcat_grades', ['userid' => $u1->id, 'amcatid' => $cm1->id]));
        $this->assertFalse($DB->record_exists('amcat_timer', ['userid' => $u1->id, 'amcatid' => $cm1->id]));
    ...
    public function test_delete_data_for_users() {
        ...
        $assertnochange = function($user, $cm) use ($DB) {
            $this->assertTrue($DB->record_exists('amcat_attempts', ['userid' => $user->id, 'amcatid' => $cm->id]));
            $this->assertTrue($DB->record_exists('amcat_grades', ['userid' => $user->id, 'amcatid' => $cm->id]));
            $this->assertTrue($DB->record_exists('amcat_timer', ['userid' => $user->id, 'amcatid' => $cm->id]));
            $this->assertTrue($DB->record_exists('amcat_branch', ['userid' => $user->id, 'amcatid' => $cm->id]));
            $this->assertTrue($DB->record_exists('amcat_overrides', ['userid' => $user->id, 'amcatid' => $cm->id]));
        };

        $assertdeleted = function($user, $cm) use ($DB) {
            $this->assertFalse($DB->record_exists('amcat_attempts', ['userid' => $user->id, 'amcatid' => $cm->id]));
            $this->assertFalse($DB->record_exists('amcat_grades', ['userid' => $user->id, 'amcatid' => $cm->id]));
            $this->assertFalse($DB->record_exists('amcat_timer', ['userid' => $user->id, 'amcatid' => $cm->id]));
            $this->assertFalse($DB->record_exists('amcat_branch', ['userid' => $user->id, 'amcatid' => $cm->id]));
            $this->assertFalse($DB->record_exists('amcat_overrides', ['userid' => $user->id, 'amcatid' => $cm->id]));
        };
        ...
        // Delete another module: no change.
        $approveduserlist = new core_privacy\local\request\approved_userlist($context3, 'mod_amcat', [$u1->id]);
        ...
        $approveduserlist = new core_privacy\local\request\approved_userlist($context1, 'mod_amcat', [$u1->id]);
        provider::delete_data_for_users($approveduserlist);
    ...
    public function test_export_data_for_user_overrides() {
        ...
        provider::export_user_data(new approved_contextlist($u1, 'mod_amcat', [$cm1ctx->id, $cm2ctx->id]));
        ...
        writer::reset();
        provider::export_user_data(new approved_contextlist($u2, 'mod_amcat', [$cm1ctx->id, $cm2ctx->id]));
        $data = writer::with_context($cm1ctx)->get_data([]);
        $this->assertNotEmpty($data);
        $data = writer::with_context($cm1ctx)->get_related_data([], 'overrides');
    ...
    public function test_export_data_for_user_timers() {
        ...
        $now = time();
        $this->create_timer($cm2, $u1, ['starttime' => $now - 2000, 'amcattime' => $now + 3600, 'completed' => 0,
            'timemodifiedoffline' => $now - 7000]);
        $this->create_timer($cm2, $u1, ['starttime' => $now - 1000, 'amcattime' => $now + 1600, 'completed' => 0]);
        $this->create_timer($cm2, $u1, ['starttime' => $now - 500, 'amcattime' => $now + 100, 'completed' => 1]);
        $this->create_timer($cm1, $u2, ['starttime' => $now - 1000, 'amcattime' => $now + 1800, 'completed' => 1]);
    ...
    public function test_export_data_for_user_attempts() {
        global $DB;
        $dg = $this->getDataGenerator();
        $lg = $dg->get_plugin_generator('mod_amcat');
        ...
        $page4 = $lg->create_question_multichoice($cm1, [
            ...
            'jumpto' => [amcat_NEXTPAGE, amcat_NEXTPAGE, amcat_THISPAGE]
        ]);
        $page4answers = array_keys($DB->get_records('amcat_answers', ['pageid' => $page4->id], 'id'));
        ...
        $this->assertEquals(get_string('endofamcat', 'mod_amcat'), $jump['went_to']);
     ...
     *
     * @param object $amcat The amcat.
     * @param object $user The user.
     * @param array $options Options.
     * @return object
     */
    protected function create_attempt($amcat, $user, array $options = []) {
        global $DB;
        ...
        $record->id = $DB->insert_record('amcat_attempts', $record);
        return $record;
    }
     ...
     * @return object
     */
    protected function create_grade($amcat, $user, array $options = []) {
        global $DB;
        $record = (object) array_merge([
            'amcatid' => $amcat->id,
            'userid' => $user->id,
        ...
        $record->id = $DB->insert_record('amcat_grades', $record);
        return $record;
    }
     ...
     * @return object
     */
    protected function create_timer($amcat, $user, array $options = []) {
        global $DB;
        $record = (object) array_merge([
            'amcatid' => $amcat->id,
            'userid' => $user->id,
        ...
        $record->id = $DB->insert_record('amcat_timer', $record);
        return $record;
    }
     ...
     * @return object
     */
    protected function create_branch($amcat, $user, array $options = []) {
        global $DB;
        $record = (object) array_merge([
            'amcatid' => $amcat->id,
            'userid' => $user->id,
        ...
        $record->id = $DB->insert_record('amcat_branch', $record);
        return $record;
    }
     ...
     * @return object
     */
    protected function create_override($amcat, $user, array $options = []) {
        global $DB;
        $record = (object) array_merge([
            'amcatid' => $amcat->id,
            'userid' => $user->id,
        ], $options);
        $record->id = $DB->insert_record('amcat_overrides', $record);
        return $record;
    }
}
