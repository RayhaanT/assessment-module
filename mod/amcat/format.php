 ...
 * @param int $contextid
 **/
function amcat_import_question_files($field, $data, $answer, $contextid) {
    global $DB;
    if (!isset($data['itemid'])) {
        return;
    }
    $text = file_save_draft_area_files($data['itemid'],
            $contextid, 'mod_amcat', 'page_' . $field . 's', $answer->id,
            array('subdirs' => false, 'maxfiles' => -1, 'maxbytes' => 0),
            $answer->$field);

    $DB->set_field("amcat_answers", $field, $text, array("id" => $answer->id));
}
...
function amcat_save_question_options($question, $amcat, $contextid) {
    ...
    switch ($question->qtype) {
        case amcat_PAGE_SHORTANSWER:
            ...
            foreach ($question->answer as $key => $dataanswer) {
                if ($dataanswer != "") {
                    ...
                    amcat_import_question_files('response', $question->feedback[$key], $answer, $contextid);
                    $answers[] = $answer->id;
            ...
            if ($maxfraction != 1) {
                ...
                $result->notice = get_string("fractionsnomax", "amcat", $maxfraction);
                return $result;
            }
            break;

        case amcat_PAGE_NUMERICAL:   // Note similarities to shortanswer.
        ...
        case amcat_PAGE_TRUEFALSE:

            // In amcat the correct answer always come first, as it was the case
            ...
            if ($question->correctanswer) {
                $answer->answer = get_string("true", "amcat");
                if (isset($question->feedbacktrue)) {
                    ...
                    $answer->id = $DB->insert_record("amcat_answers", $answer);
                    amcat_import_question_files('response', $question->feedbacktrue, $answer, $contextid);
                }
            } else {
                $answer->answer = get_string("false", "amcat");
                if (isset($question->feedbackfalse)) {
                    ...
                    $answer->id = $DB->insert_record("amcat_answers", $answer);
                    amcat_import_question_files('response', $question->feedbackfalse, $answer, $contextid);
                }
            }
        ...
        case amcat_PAGE_MULTICHOICE:
            ...
            foreach ($question->answer as $key => $dataanswer) {
                if ($dataanswer != "") {
                    ...
                    $answer->responseformat = $question->feedback[$key]['format'];
                    $answer->id = $DB->insert_record("amcat_answers", $answer);
                    amcat_import_question_files('answer', $dataanswer, $answer, $contextid);
                    amcat_import_question_files('response', $question->feedback[$key], $answer, $contextid);
            ...
            if ($question->single) {
                ...
                if ($totalfraction != 1) {
                    ...
                    $result->notice = get_string("fractionsaddwrong", "amcat", $totalfraction);
                    return $result;
                }
            }
        break;

        case amcat_PAGE_MATCHING:

            $subquestions = array();

            // The first answer should always be the correct answer
            $correctanswer = clone($defaultanswer);
            $correctanswer->answer = get_string('thatsthecorrectanswer', 'amcat');
            $correctanswer->jumpto = amcat_NEXTPAGE;
            $correctanswer->score = 1;
            $DB->insert_record("amcat_answers", $correctanswer);

            // The second answer should always be the wrong answer
            $wronganswer = clone($defaultanswer);
            $wronganswer->answer = get_string('thatsthewronganswer', 'amcat');
            $DB->insert_record("amcat_answers", $wronganswer);
            ...
            foreach ($question->subquestions as $key => $questiontext) {
                ...
                if (!empty($questiontext) and !empty($answertext)) {
                    ...
                    $answer->id = $DB->insert_record("amcat_answers", $answer);
                    amcat_import_question_files('answer', $questiontext, $answer, $contextid);
            ...
            if (count($subquestions) < 3) {
                $result->notice = get_string("notenoughsubquestions", "amcat");
                return $result;
            }
            break;

        case amcat_PAGE_ESSAY:
            $answer = new stdClass();
            $answer->amcatid = $question->amcatid;
...
class qformat_default {
    ...
    var $qtypeconvert = array('numerical'   => amcat_PAGE_NUMERICAL,
                               'multichoice' => amcat_PAGE_MULTICHOICE,
                               'truefalse'   => amcat_PAGE_TRUEFALSE,
                               'shortanswer' => amcat_PAGE_SHORTANSWER,
                               'match'       => amcat_PAGE_MATCHING,
                               'essay'       => amcat_PAGE_ESSAY
                              );
    ...
    function importprocess($filename, $amcat, $pageid) {
        global $DB, $OUTPUT;
    ...
    /// Processes a given file.  There's probably little need to change this
        ...
        echo $OUTPUT->notification(get_string('importcount', 'amcat',
        ...
        if ($pageid == 0) {
            ...
            $updateamcatpage = $DB->get_record('amcat_pages', array('amcatid' => $amcat->id, 'prevpageid' => 0));
        } else {
            $updateamcatpage = $DB->get_record('amcat_pages', array('amcatid' => $amcat->id, 'id' => $pageid));
        }
                ...
                case 'essay' :
                    ...
                    if ($pageid) {
                        ...
                        if (!$page = $DB->get_record_select("amcat_pages", "amcatid = :amcatid AND prevpageid = :prevpageid", $params)) {
                            ...
                            $newpageid = $DB->insert_record("amcat_pages", $newpage);
                            // update the linked list
                            $DB->set_field("amcat_pages", "prevpageid", $newpageid, array("id" => $page->id));
                        }
                    }
                    ...
                    if (isset($question->questiontextitemid)) {
                        $questiontext = file_save_draft_area_files($question->questiontextitemid,
                                $this->importcontext->id, 'mod_amcat', 'page_contents', $newpageid,
                                null , $question->questiontext);
                        // Update content with recoded urls.
                        $DB->set_field("amcat_pages", "contents", $questiontext, array("id" => $newpageid));
                    }

                    // Now to save all the answers and type-specific options

                    $question->amcatid = $amcat->id; // needed for foreign key
                    $question->qtype = $this->qtypeconvert[$question->qtype];
                    $result = amcat_save_question_options($question, $amcat, $this->importcontext->id);
        ...
        if (!empty($updateamcatpage)) {
            if ($addquestionontop) {
                $DB->set_field("amcat_pages", "prevpageid", $pageid, array("id" => $updateamcatpage->id));
            } else {
                $DB->set_field("amcat_pages", "prevpageid", $pageid, array("id" => $updateamcatpage->nextpageid));
            }
        }
        if ($unsupportedquestions) {
            echo $OUTPUT->notification(get_string('unknownqtypesnotimported', 'amcat', $unsupportedquestions));
        }
        return true;
    }
    ...
    /**
     * Since the amcat module tries to re-use the question bank import classes in
