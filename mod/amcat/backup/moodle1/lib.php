...
/**
 * amcat conversion handler
 */
class moodle1_mod_amcat_handler extends moodle1_mod_handler {
     ...
     * defined.
     *
     * Note that the path /MOODLE_BACKUP/COURSE/MODULES/MOD/amcat does not
    ...
    public function get_paths() {
        return array(
            new convert_path(
                'amcat', '/MOODLE_BACKUP/COURSE/MODULES/MOD/amcat',
                array(
                    'renamefields' => array(
            ...
            new convert_path(
                'amcat_page', '/MOODLE_BACKUP/COURSE/MODULES/MOD/amcat/PAGES/PAGE',
                array(
                    'newfields' => array(
                        'contentsformat' => FORMAT_MOODLE,
                        'nextpageid' => 0, //set to default to the next sequencial page in process_amcat_page()
                        'prevpageid' => 0
                    ),
                )
            ),
            new convert_path(
                'amcat_pages', '/MOODLE_BACKUP/COURSE/MODULES/MOD/amcat/PAGES'
            ),
            new convert_path(
                'amcat_answer', '/MOODLE_BACKUP/COURSE/MODULES/MOD/amcat/PAGES/PAGE/ANSWERS/ANSWER',
                array(
                    'newfields' => array(
                        'answerformat' => 0,
                        'responseformat' => 0,
    ...
    /**
     * This is executed every time we have one /MOODLE_BACKUP/COURSE/MODULES/MOD/amcat
     * data available
     */
    public function process_amcat($data) {
        ...
        $this->fileman = $this->converter->get_file_manager($contextid, 'mod_amcat');
        ...
        if (!empty($data['mediafile']) and strpos($data['mediafile'], '://') === false) {
            ...
            try {
                ...
                $this->log('error migrating amcat mediafile', backup::LOG_WARNING, 'course_files/'.$data['mediafile']);
            }
        }

        // start writing amcat.xml
        $this->open_xml_writer("activities/amcat_{$this->moduleid}/amcat.xml");
        $this->xmlwriter->begin_tag('activity', array('id' => $instanceid, 'moduleid' => $this->moduleid,
            'modulename' => 'amcat', 'contextid' => $contextid));
        $this->xmlwriter->begin_tag('amcat', array('id' => $instanceid));
    ...
    public function on_amcat_pages_start() {
        $this->xmlwriter->begin_tag('pages');
    }

    /**
     * This is executed every time we have one /MOODLE_BACKUP/COURSE/MODULES/MOD/amcat/PAGES/PAGE
     * data available
     */
    public function process_amcat_page($data) {
        global $CFG;
    ...
    /**
     * This is executed every time we have one /MOODLE_BACKUP/COURSE/MODULES/MOD/amcat/PAGES/PAGE/ANSWERS/ANSWER
     * data available
     */
    public function process_amcat_answer($data) {
        ...
        // upgrade step 2010121400 from mod/amcat/db/upgrade.php
        $this->answers[] = $data;
    }

    public function on_amcat_page_end() {
        ...
        if ( $firstbatch || $nextbatch ) { //we can write out 1 page atleast
            if ($this->prevpageid == 0) {
                // start writing with n-2 page (relative to this on_amcat_page_end() call)
                $pg1 = $this->pages[1];
                $pg0 = $this->pages[0];
    ...
    public function on_amcat_pages_end() {
        if ($this->pages) {
    ...
    /**
     * This is executed when we reach the closing </MOD> tag of our 'amcat' path
     */
    public function on_amcat_end() {
        // Append empty <overrides> subpath element.
        $this->write_xml('overrides', array());

        // finish writing amcat.xml
        $this->xmlwriter->end_tag('amcat');
        $this->xmlwriter->end_tag('activity');
        $this->close_xml_writer();

        // write inforef.xml
        $this->open_xml_writer("activities/amcat_{$this->moduleid}/inforef.xml");
    ...
    protected function write_single_page_xml($page, $prevpageid=0, $nextpageid=0) {
        ...
        //effectively on_amcat_answers_end(), where we write out answers for current page.
