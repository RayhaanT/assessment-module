...
$url = new moodle_url('/mod/amcat/report.php', array('id'=>$id));
...
if ($action == 'reportoverview') {
    $PAGE->navbar->add(get_string('reports', 'amcat'));
    $PAGE->navbar->add(get_string('overview', 'amcat'));
}
...
if ($action === 'delete') {
    ...
    if (has_capability('mod/amcat:edit', $context) and $form = data_submitted() and confirm_sesskey()) {
        ...
        if (!empty($form->attempts)) {
            foreach ($form->attempts as $userid => $tries) {
                ...
                /// Clean up the timer table by removing using the order - this is silly, it should be linked to specific attempt (skodak)
                    $timers = $amcat->get_user_timers($userid, 'starttime', 'id', $try, 1);
                    if ($timers) {
                        $timer = reset($timers);
                        $DB->delete_records('amcat_timer', array('id' => $timer->id));
                    }

                    $params = array ("userid" => $userid, "amcatid" => $amcat->id);
                    // Remove the grade from the grades tables - this is silly, it should be linked to specific attempt (skodak).
                    $grades = $DB->get_records_sql("SELECT id FROM {amcat_grades}
                                                     WHERE userid = :userid AND amcatid = :amcatid
                                                  ORDER BY completed", $params, $try, 1);

                    if ($grades) {
                        $grade = reset($grades);
                        $DB->delete_records('amcat_grades', array('id' => $grade->id));
                    }

                /// Remove attempts and update the retry number
                    $DB->delete_records('amcat_attempts', array('userid' => $userid, 'amcatid' => $amcat->id, 'retry' => $try));
                    $DB->execute("UPDATE {amcat_attempts} SET retry = retry - 1 WHERE userid = ? AND amcatid = ? AND retry > ?", array($userid, $amcat->id, $try));

                /// Remove seen branches and update the retry number
                    $DB->delete_records('amcat_branch', array('userid' => $userid, 'amcatid' => $amcat->id, 'retry' => $try));
                    $DB->execute("UPDATE {amcat_branch} SET retry = retry - 1 WHERE userid = ? AND amcatid = ? AND retry > ?", array($userid, $amcat->id, $try));

                /// update central gradebook
                    amcat_update_grades($amcat, $userid);
    ...
    **************************************************************************/
    ...
    list($table, $data) = amcat_get_overview_report_table_and_data($amcat, $currentgroup);

    if ($table === false) {
        echo $amcatoutput->header($amcat, $cm, $action, false, null, get_string('noamcatattempts', 'amcat'));
        if (!empty($currentgroup)) {
            $groupname = groups_get_group_name($currentgroup);
            echo $OUTPUT->notification(get_string('noamcatattemptsgroup', 'amcat', $groupname));
        } else {
            echo $OUTPUT->notification(get_string('noamcatattempts', 'amcat'));
        }
    ...
    echo $amcatoutput->header($amcat, $cm, $action, false, null, get_string('overview', 'amcat'));
    groups_print_activity_menu($cm, $url);
    ...
    // Show bulk actions when user has capability to edit the amcat.
    if (has_capability('mod/amcat:edit', $context)) {
        $reporturl = new moodle_url('/mod/amcat/report.php');
        $formid  = 'mod-amcat-report-form';
        ...
        $bulkactions->set_label(get_string('withselectedattempts', 'amcat'));
        $bulkactions->disabled = true;
        $bulkactions->attributes = [
            'data-action' => 'toggle',
            'data-togglegroup' => 'amcat-attempts',
            'data-toggle' => 'action',
        ];
    ...
    if ($data->amcatscored) {
        if ($data->numofattempts == 0) {
            $data->avescore = get_string("notcompleted", "amcat");
        } else {
            $data->avescore = format_float($data->avescore, 2) . '%';
        }
        if ($data->highscore === null) {
            $data->highscore = get_string("notcompleted", "amcat");
        } else {
            $data->highscore .= '%';
        }
        if ($data->lowscore === null) {
            $data->lowscore = get_string("notcompleted", "amcat");
        } else {
            $data->lowscore .= '%';
        }

        // Display the full stats for the amcat.
        echo $OUTPUT->heading(get_string('amcatstats', 'amcat'), 3);
        $stattable = new html_table();
        $stattable->head = array(get_string('averagescore', 'amcat'), get_string('averagetime', 'amcat'),
                                get_string('highscore', 'amcat'), get_string('lowscore', 'amcat'),
                                get_string('hightime', 'amcat'), get_string('lowtime', 'amcat'));
        ...
        // Display simple stats for the amcat.
        echo $OUTPUT->heading(get_string('amcatstats', 'amcat'), 3);
        $stattable = new html_table();
        $stattable->head = array(get_string('averagetime', 'amcat'), get_string('hightime', 'amcat'),
                                get_string('lowtime', 'amcat'));
...
**************************************************************************/
    echo $amcatoutput->header($amcat, $cm, $action, false, null, get_string('detailedstats', 'amcat'));
    groups_print_activity_menu($cm, $url);
    ...
    list($answerpages, $userstats) = amcat_get_user_detailed_report_data($amcat, $userid, $try);
    ...
    if (!empty($userid)) {
        // if looking at a students try, print out some basic stats at the top
            ...
            //print_heading(get_string("studentattemptamcat", "amcat", $headingobject));
        echo $OUTPUT->heading(get_string('attempt', 'amcat', $try+1), 3);
        ...
        if (empty($userstats->gradeinfo)) {
            ...
            $table->data[] = array(get_string("notcompleted", "amcat"));
        } else {
            $user = $DB->get_record('user', array('id' => $userid));

            $gradeinfo = amcat_grade($amcat, $try, $user->id);

            $table->data[] = array(get_string('name').':', $OUTPUT->user_picture($user, array('courseid'=>$course->id)).fullname($user, true));
            $table->data[] = array(get_string("timetaken", "amcat").":", format_time($userstats->timetotake));
            $table->data[] = array(get_string("completed", "amcat").":", userdate($userstats->completed));
            $table->data[] = array(get_string('rawgrade', 'amcat').':', $userstats->gradeinfo->earned.'/'.$userstats->gradeinfo->total);
            $table->data[] = array(get_string("grade", "amcat").":", $userstats->grade."%");
        }
    ...
    foreach ($answerpages as $page) {
        ...
        $table->head = array($fontstart2.$page->qtype.": ".format_string($page->title).$fontend2, $fontstart2.get_string("classstats", "amcat").$fontend2);
        $table->data[] = array($fontstart.get_string("question", "amcat").": <br />".$fontend.$fontstart2.$page->contents.$fontend2, " ");
        $table->data[] = array($fontstart.get_string("answer", "amcat").":".$fontend, ' ');
        ...
        if (!empty($page->answerdata) && !empty($page->answerdata->answers)) {
            ...
            if (isset($page->answerdata->response)) {
                $table->data[] = array($fontstart.get_string("response", "amcat").": <br />".$fontend
                        .$fontstart2.$page->answerdata->response.$fontend2, " ");
            }
            $table->data[] = array($page->answerdata->score, " ");
        } else {
            $table->data[] = array(get_string('didnotanswerquestion', 'amcat'), " ");
        }
