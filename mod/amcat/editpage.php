...
if (!empty($returnto)) {
    ...
    $returnto = new moodle_url('/mod/amcat/edit.php', array('id' => $id));
    $returnto->set_anchor('amcat-' . $pageid);
}
...
$PAGE->set_url('/mod/amcat/editpage.php', array('pageid'=>$pageid, 'id'=>$id, 'qtype'=>$qtype));
$PAGE->set_pagelayout('admin');

if ($edit) {
    $editpage = amcat_page::load($pageid, $amcat);
...
$jumpto = amcat_page::get_jumptooptions($pageid, $amcat);
...
// will have a different name (e.g. _qf__amcat_add_page_form_selection
// versus _qf__amcat_add_page_form_multichoice). This causes confusion
...
if ($edit) {
    ...
    $data = file_prepare_standard_editor($data, 'contents', $editoroptions, $context, 'mod_amcat', 'page_contents',  $editpage->id);
    ...
    foreach ($answers as $answer) {
        ...
        if (is_array($data->$answereditor)) {
            ...
            if ($mform->get_answer_format() === amcat_ANSWER_HTML) {
                ...
                $answertext = file_prepare_draft_area($answerdraftid, $PAGE->cm->context->id,
                        'mod_amcat', 'page_answers', $answer->id, $editoroptions, $answerdata['text']);
        ...
        if (is_array($data->$responseeditor)) {
            ...
            if ($mform->get_response_format() === amcat_ANSWER_HTML) {
                ...
                $responsetext = file_prepare_draft_area($responsedraftid, $PAGE->cm->context->id,
                        'mod_amcat', 'page_responses', $answer->id, $editoroptions, $responsedata['text']);
    ...
    // Let the amcat pages make updates if required.
    $data = $editpage->update_form_data($data);

    $mform->set_data($data);
    $PAGE->navbar->add(get_string('edit'), new moodle_url('/mod/amcat/edit.php', array('id'=>$id)));
    $PAGE->navbar->add(get_string('editingquestionpage', 'amcat', get_string($mform->qtypestring, 'amcat')));
} else {
    ...
    $mform->construction_override($pageid, $amcat);
    ...
    $mform->set_data($data);
    $PAGE->navbar->add(get_string('addanewpage', 'amcat'), $PAGE->url);
    if ($qtype !== 'unknown') {
        $PAGE->navbar->add(get_string($mform->qtypestring, 'amcat'));
    }
}
...
if ($data = $mform->get_data()) {
    require_sesskey();
    if ($edit) {
        $data->amcatid = $data->id;
        $data->id = $data->pageid;
        unset($data->pageid);
        unset($data->edit);
        $editpage->update($data, $context, $PAGE->course->maxbytes);
    } else {
        $editpage = amcat_page::create($data, $amcat, $context, $PAGE->course->maxbytes);
    }
    redirect($returnto);
}

$amcatoutput = $PAGE->get_renderer('mod_amcat');
echo $amcatoutput->header($amcat, $cm, '', false, null, get_string('edit', 'amcat'));
$mform->display();
