...
/** Essay question type */
define("amcat_PAGE_ESSAY", "10");

class amcat_page_type_essay extends amcat_page {

    protected $type = amcat_page::TYPE_QUESTION;
    protected $typeidstring = 'essay';
    protected $typeid = amcat_PAGE_ESSAY;
    ...
    public function display($renderer, $attempt) {
        ...
        $mform = new amcat_display_answer_form_essay($CFG->wwwroot.'/mod/amcat/continue.php', $options);
        ...
        $data = file_prepare_standard_editor($data, 'answer', $options['editoroptions'],
            $context, 'mod_amcat', 'essay_answers');
        $mform->set_data($data);
    ...
    public function create_answers($properties) {
        ...
        $newanswer->id = $DB->insert_record("amcat_answers", $newanswer);
        $answers = array($newanswer->id => new amcat_page_answer($newanswer));
    ...
    public function on_after_write_attempt($attempt, $result) {
        ...
        if ($formdata = $result->postdata) {
            ...
            $formdata = file_postupdate_standard_editor($formdata, 'answer', $editoroptions,
                $editoroptions['context'], 'mod_amcat', 'essay_answers', $attempt->id);
    ...
    public function format_answer($answer, $context, $answerformat, $options = []) {
        $answer = file_rewrite_pluginfile_urls($answer, 'pluginfile.php', $context->id,
            'mod_amcat', 'essay_answers', $options->attemptid);
    ...
    public function update($properties, $context = null, $maxbytes = null) {
        ...
        if (!array_key_exists(0, $this->answers)) {
            ...
            $this->answers[0]->amcatid = $this->amcat->id;
        ...
        if (!isset($this->answers[0]->id)) {
            $this->answers[0]->id =  $DB->insert_record("amcat_answers", $this->answers[0]);
        } else {
            $DB->update_record("amcat_answers", $this->answers[0]->properties());
        }
    ...
    public function report_answers($answerpage, $answerdata, $useranswer, $pagestats, &$i, &$n) {
        ...
        $context = context_module::instance($PAGE->cm->id);
        foreach ($answers as $answer) {
            $hasattempts = $DB->record_exists('amcat_attempts', ['answerid' => $answer->id]);
            if ($useranswer != null) {
                ...
                $essayinfo->answer = file_rewrite_pluginfile_urls($essayinfo->answer, 'pluginfile.php',
                    $context->id, 'mod_amcat', 'essay_answers', $useranswer->id);

                if ($essayinfo->response == null) {
                    $answerdata->response = get_string("nocommentyet", "amcat");
                } else {
                    $essayinfo->response = file_rewrite_pluginfile_urls($essayinfo->response, 'pluginfile.php',
                            $answerpage->context->id, 'mod_amcat', 'essay_responses', $useranswer->id);
                ...
                if (isset($pagestats[$this->properties->id])) {
                    ...
                    $percent = get_string("averagescore", "amcat").": ". $percent ."%";
                } else {
                ...
                if ($essayinfo->graded) {
                    if ($this->amcat->custom) {
                        $answerdata->score = get_string("pointsearned", "amcat").": " . $essayinfo->score;
                    ...
                    $answerdata->score = get_string("havenotgradedyet", "amcat");
                }
            } else {
                $essayinfo = new stdClass();
                if ($hasattempts && has_capability('mod/amcat:grade', $answerpage->context)) {
                    $essayinfo->answer = html_writer::link(new moodle_url("/mod/amcat/essay.php",
                        ['id' => $PAGE->cm->id]), get_string("viewessayanswers", "amcat"));
                } else {
                    $essayinfo->answer = "";
                }
            ...
            if (isset($pagestats[$this->properties->id])) {
                ...
                $avescore = get_string("averagescore", "amcat").": ". $avescore ;
            } else {
                $avescore = $hasattempts ? get_string("essaynotgradedyet", "amcat") :
                        get_string("nooneansweredthisquestion", "amcat");
            }
...
class amcat_add_page_form_essay extends amcat_add_page_form_base {
...
class amcat_display_answer_form_essay extends moodleform {
    ...
    public function definition() {
        ...
        $useranswer = '';
        $useranswerraw = '';
        if (isset($this->_customdata['amcatid'])) {
            $amcatid = $this->_customdata['amcatid'];
            if (isset($USER->modattempts[$amcatid]->useranswer) && !empty($USER->modattempts[$amcatid]->useranswer)) {
                $attrs = array('disabled' => 'disabled');
                $hasattempt = true;
                $useranswertemp = amcat_page_type_essay::extract_useranswer($USER->modattempts[$amcatid]->useranswer);
        ...
        if ($hasattempt) {
            ...
            $mform->addElement('html', $OUTPUT->container(get_string('youranswer', 'amcat'), 'youranswer'));
            $useranswer = file_rewrite_pluginfile_urls($useranswer, 'pluginfile.php', $editoroptions['context']->id,
                'mod_amcat', 'essay_answers', $this->_customdata['attemptid']);
            $mform->addElement('html', $OUTPUT->container($useranswer, 'reviewessay'));
            $this->add_action_buttons(null, get_string("nextpage", "amcat"));
        } else {
            $mform->addElement('editor', 'answer_editor', get_string('youranswer', 'amcat'), null, $editoroptions);
