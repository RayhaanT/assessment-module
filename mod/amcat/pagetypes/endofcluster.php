...
defined('MOODLE_INTERNAL') || die();
 ...
 /** End of Cluster page */
define("amcat_PAGE_ENDOFCLUSTER",   "31");

class amcat_page_type_endofcluster extends amcat_page {

    protected $type = amcat_page::TYPE_STRUCTURE;
    protected $typeidstring = 'endofcluster';
    protected $typeid = amcat_PAGE_ENDOFCLUSTER;
    ...
    public function redirect_to_next_page($canmanage, $redirect) {
        ...
        if ($this->properties->nextpageid == 0) {
            $nextpageid = amcat_EOL;
        } else {
            $nextpageid = $this->properties->nextpageid;
        }
        if ($redirect) {
            redirect(new moodle_url('/mod/amcat/view.php', array('id' => $PAGE->cm->id, 'pageid' => $nextpageid)));
            die;
        }
        return $nextpageid;
    }
    ...
    public function override_next_page() {
        global $DB;
        $jump = $DB->get_field("amcat_answers", "jumpto", array("pageid" => $this->properties->id, "amcatid" => $this->amcat->id));
        if ($jump == amcat_NEXTPAGE) {
            if ($this->properties->nextpageid == 0) {
                return amcat_EOL;
            } else {
    ...
    public function add_page_link($previd) {
        global $PAGE, $CFG;
        if ($previd != 0) {
            $addurl = new moodle_url('/mod/amcat/editpage.php', array('id'=>$PAGE->cm->id, 'pageid'=>$previd, 'sesskey'=>sesskey(), 'qtype'=>amcat_PAGE_ENDOFCLUSTER));
            return array('addurl'=>$addurl, 'type'=>amcat_PAGE_ENDOFCLUSTER, 'name'=>get_string('addendofcluster', 'amcat'));
        }
        return false;
    }
...
class amcat_add_page_form_endofcluster extends amcat_add_page_form_base {

    public $qtype = amcat_PAGE_ENDOFCLUSTER;
    ...
    public function construction_override($pageid, amcat $amcat) {
        ...
        $newpage->title = get_string("endofclustertitle", "amcat");
        $newpage->contents = get_string("endofclustertitle", "amcat");
        ...
        $newanswer->jumpto = amcat_NEXTPAGE;
        $newanswerid = $DB->insert_record("amcat_answers", $newanswer);
        $amcat->add_message(get_string('addedendofcluster', 'amcat'), 'notifysuccess');
