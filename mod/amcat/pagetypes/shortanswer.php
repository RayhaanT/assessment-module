...
define("amcat_PAGE_SHORTANSWER",   "1");

class amcat_page_type_shortanswer extends amcat_page {

    protected $type = amcat_page::TYPE_QUESTION;
    protected $typeidstring = 'shortanswer';
    protected $typeid = amcat_PAGE_SHORTANSWER;
    ...
    public function display($renderer, $attempt) {
        ...
        $mform = new amcat_display_answer_form_shortanswer($CFG->wwwroot.'/mod/amcat/continue.php', array('contents'=>$this->get_contents(), 'amcatid'=>$this->amcat->id));
        $data = new stdClass;
        $data->id = $PAGE->cm->id;
    ...
    public function create_answers($properties) {
        if (isset($properties->enableotheranswers) && $properties->enableotheranswers) {
            ...
            $properties->answer_editor[$wrongkey] = amcat_OTHER_ANSWERS;
        }
    ...
    public function check_answer() {
        ...
        $mform = new amcat_display_answer_form_shortanswer($CFG->wwwroot.'/mod/amcat/continue.php', array('contents'=>$this->get_contents()));
        $data = $mform->get_data();
        require_sesskey();
        ...
        foreach ($answers as $answer) {
            ...
            if ((!$this->amcat->custom && $this->amcat->jumpto_is_correct($this->properties->id, $answer->jumpto)) or ($this->amcat->custom && $answer->score > 0) ) {
                if (!$useregexp) { // we are using 'normal analysis', which ignores case
        ...
        if ($result->answerid == 0) {
            ...
            if (strpos($lastanswer->answer, amcat_OTHER_ANSWERS) !== false) {
    ...
    public function option_description_string() {
        if ($this->properties->qoption) {
            return " - ".get_string("casesensitive", "amcat");
        }
            ...
            if ($useranswer == null && $i == 0) {
                ...
                if (isset($pagestats[$this->properties->id])) {
                    ...
                    foreach ($stats as $valentered => $ntimes) {
                        ...
                        $percent .= "% ".get_string("enteredthis", "amcat");
                        $answerdata->answers[] = array($data, $percent);
                    }
                } else {
                    $answerdata->answers[] = array(get_string("nooneansweredthisquestion", "amcat"), " ");
                }
                $i++;
                ...
                if (isset($pagestats[$this->properties->id][$useranswer->useranswer])) {
                    ...
                    $percent = get_string("nooneenteredthis", "amcat");
                }
                $answerdata->answers[] = array($data, $percent);
                ...
                if ($answer->id == $useranswer->answerid) {
                    ...
                    $answerdata->response = get_string("thatsthewronganswer", "amcat");
                    if ($this->amcat->custom) {
                        $answerdata->score = get_string("pointsearned", "amcat").": 0";
                    } else {
                        $answerdata->score = get_string("didnotreceivecredit", "amcat");
                    }
                }
    ...
    public function update_form_data(stdClass $data) : stdClass {
        ...
        if (strpos($lastanswer, amcat_OTHER_ANSWERS) !== false) {
            $data->{'answer_editor[' . ($this->amcat->maxanswers + 1) . ']'} =
                    $data->{'answer_editor[' . ($answercount - 1) . ']'};
            $data->{'response_editor[' . ($this->amcat->maxanswers + 1) . ']'} =
                    $data->{'response_editor[' . ($answercount - 1) . ']'};
            $data->{'jumpto[' . ($this->amcat->maxanswers + 1) . ']'} = $data->{'jumpto[' . ($answercount - 1) . ']'};
            $data->{'score[' . ($this->amcat->maxanswers + 1) . ']'} = $data->{'score[' . ($answercount - 1) . ']'};
...
class amcat_add_page_form_shortanswer extends amcat_add_page_form_base {
    ...
    public function custom_definition() {
        ...
        $this->_form->addElement('checkbox', 'qoption', get_string('options', 'amcat'), get_string('casesensitive', 'amcat')); //oh my, this is a regex option!
        $this->_form->setDefault('qoption', 0);
        $this->_form->addHelpButton('qoption', 'casesensitive', 'amcat');

        $answercount = $this->_customdata['amcat']->maxanswers;
        ...
        // Other answer jump.
        $this->_form->addElement('header', 'wronganswer', get_string('allotheranswers', 'amcat'));
        $newcount = $answercount + 1;
        $this->_form->addElement('advcheckbox', 'enableotheranswers', get_string('enabled', 'amcat'));
        $this->add_response($newcount);
        $this->add_jumpto($newcount, get_string('allotheranswersjump', 'amcat'), amcat_NEXTPAGE);
        $this->add_score($newcount, get_string('allotheranswersscore', 'amcat'), 0);
    }
}

class amcat_display_answer_form_shortanswer extends moodleform {
    ...
    public function definition() {
        ...
        if (isset($this->_customdata['amcatid'])) {
            ...
            if (isset($USER->modattempts[$amcatid]->useranswer)) {
        ...
        if ($placeholder) {
            ...
            $mform->addElement('text', 'answer', get_string('youranswer', 'amcat'), $attrs);
