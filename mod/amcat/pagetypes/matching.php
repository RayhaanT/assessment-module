...
define("amcat_PAGE_MATCHING",      "5");

class amcat_page_type_matching extends amcat_page {

    protected $type = amcat_page::TYPE_QUESTION;
    protected $typeid = amcat_PAGE_MATCHING;
    ...
    protected function make_answer_form($attempt=null) {
        ...
        if (isset($USER->modattempts[$this->amcat->id]) && !empty($attempt->useranswer)) {
        ...
        $params = array('answers'=>$answers, 'useranswers'=>$useranswers, 'responseoptions'=>$responseoptions, 'amcatid'=>$this->amcat->id, 'contents'=>$this->get_contents());
        $mform = new amcat_display_answer_form_matching($action, $params);
        return $mform;
    }
    ...
    public function create_answers($properties) {
        ...
        $this->amcat->maxanswers = $this->amcat->maxanswers + 2;
    ...
    public function option_description_string() {
        return get_string("firstanswershould", "amcat");
    }
        ...
        foreach ($answers as $answer) {
            ...
            if ($n < 2) {
                if ($answer->answer != null) {
                    $cells = array();
                    if ($n == 0) {
                        $cells[] = '<label>' . get_string('correctresponse', 'amcat') . '</label>';
                    } else {
                        $cells[] = '<label>' . get_string('wrongresponse', 'amcat') . '</label>';
                    }
                ...
                if ($n == 0) {
                    $cells = array();
                    $cells[] = '<label>' . get_string('correctanswerscore', 'amcat') . '</label>: ';
                    $cells[] = $answer->score;
                    $table->data[] = new html_table_row($cells);

                    $cells = array();
                    $cells[] = '<label>' . get_string('correctanswerjump', 'amcat') . '</label>: ';
                    $cells[] = $this->get_jump_name($answer->jumpto);
                    $table->data[] = new html_table_row($cells);
                } elseif ($n == 1) {
                    $cells = array();
                    $cells[] = '<label>' . get_string('wronganswerscore', 'amcat') . '</label>: ';
                    $cells[] = $answer->score;
                    $table->data[] = new html_table_row($cells);

                    $cells = array();
                    $cells[] = '<label>' . get_string('wronganswerjump', 'amcat') . '</label>: ';
                    $cells[] = $this->get_jump_name($answer->jumpto);
                    $table->data[] = new html_table_row($cells);
                }
                ...
                $cells = array();
                $cells[] = '<label>' . get_string('matchesanswer', 'amcat') . " {$i}</label>: \n";
    ...
    public function report_answers($answerpage, $answerdata, $useranswer, $pagestats, &$i, &$n) {
        ...
        foreach ($answers as $answer) {
            if ($n == 0 && $useranswer != null && $useranswer->correct) {
                ...
                $data = '<label class="accesshide" for="answer_' . $n . '">' . get_string('answer', 'amcat') . '</label>';
                $data .= strip_tags(format_string($answer->answer)) . ' ';
                if ($useranswer != null) {
                    $userresponse = explode(",", $useranswer->useranswer);
                    $data .= '<label class="accesshide" for="stu_answer_response_' . $n . '">' . get_string('matchesanswer', 'amcat') . '</label>';
                    ...
                    $data .= '<label class="accesshide" for="answer_response_' . $n . '">' . get_string('matchesanswer', 'amcat') . '</label>';
                ...
                if ($n == 2) {
                    if (isset($pagestats[$this->properties->id])) {
                        ...
                        $percent .= "% ".get_string("answeredcorrectly", "amcat");
                    } else {
                        $percent = get_string("nooneansweredthisquestion", "amcat");
                    }
                } else {
                    $percent = '';
                }
    ...
    public function get_jumps() {
        ...
        $jumps = array();
        if ($answers = $DB->get_records("amcat_answers", array("amcatid" => $this->amcat->id, "pageid" => $this->properties->id), 'id', '*', 0, 2)) {
            foreach ($answers as $answer) {
...
class amcat_add_page_form_matching extends amcat_add_page_form_base {
    ...
    public function custom_definition() {
        ...
        $this->_form->addElement('header', 'correctresponse', get_string('correctresponse', 'amcat'));
        $this->_form->addElement('editor', 'answer_editor[0]', get_string('correctresponse', 'amcat'),
        ...
        $this->_form->addElement('header', 'wrongresponse', get_string('wrongresponse', 'amcat'));
        $this->_form->addElement('editor', 'answer_editor[1]', get_string('wrongresponse', 'amcat'),
                array('rows' => '4', 'columns' => '80'),
                array('noclean' => true, 'maxfiles' => EDITOR_UNLIMITED_FILES, 'maxbytes' => $this->_customdata['maxbytes']));
        ...
        for ($i = 2; $i < $this->_customdata['amcat']->maxanswers+2; $i++) {
            $this->_form->addElement('header', 'matchingpair'.($i-1), get_string('matchingpair', 'amcat', $i-1));
            $this->add_answer($i, null, ($i < 4), amcat_ANSWER_HTML);
            $required = ($i < 4);
            $label = get_string('matchesanswer','amcat');
            $count = $i;
...
class amcat_display_answer_form_matching extends moodleform {
    ...
    public function definition() {
        ...
        foreach ($answers as $answer) {
            ...
            if ($answer->response != null) {
                ...
                $answer = amcat_page_type_matching::rewrite_answers_urls($answer);
