...
class block_intelliboard extends block_base {
    ...
    public function get_content() {
       ...
       // $courseid = 2;
        ...
        $defaultcourses = 0;
        if(!empty($mycoures)){
            //print_object(current($mycoures)->courseid);exit();
        ...
        $widgets .= html_writer::start_tag('form', array('id'=>'intelliboard_learners_performance_form'));
        //$widgets .= html_writer::tag('h4', 'Select course to load reports charts');
        $widgets .= html_writer::select($mycourselist, 'course', $courseid, '',array('onchange' => 'this.form.submit(this.value)'));
    ...
    /*    if (isloggedin() and !$canupdate) {
            ...
            $icon = html_writer::empty_tag('img', array('src'=>$OUTPUT->image_url('i/user')->out(false),'id'=>'intelliboard-navcontent'));
        ...
        if (isloggedin() and $canupdate) {
            ...
            $icon = html_writer::empty_tag('img', array('src'=>$OUTPUT->image_url('i/outcomes')->out(false),'id'=>'intelliboard-navcontent'));
        ...
        if (is_siteadmin() and $canupdate) {
            ...
            $icon = html_writer::empty_tag('img', array('src'=>$OUTPUT->image_url('i/scales')->out(false),'id'=>'intelliboard-navcontent'));
            $menu .= html_writer::link($url, $icon .' '. get_string('s1', 'block_intelliboard'), array('target' => '_blank', 'class' => 'external-link'));

            $icon = html_writer::empty_tag('img', array('src'=>$OUTPUT->image_url('i/publish')->out(false),'id'=>'intelliboard-navcontent'));
...
*/
        ...
        $content .= html_writer::empty_tag('img', array('src'=>$OUTPUT->image_url('a/view_list_active')->out(false),'id'=>'intelliboard-navcontent'));
        $content .= html_writer::empty_tag('img', array('src' => $OUTPUT->image_url('e/fullscreen')->out(false), 'id' => 'intelliboard-resize'));
