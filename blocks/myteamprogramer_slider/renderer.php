 ...
 * @license http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class block_myteamprogramer_slider_renderer extends plugin_renderer_base {
     ...
     * @return string
     */
    public function block_myteamprogramer_slider_css_file_as_html($filerelativepath) {
        global $CFG;
     ...
     * @return string
     */
    public function block_myteamprogramer_slider_js_file_as_html($filerelativepath) {
        global $CFG;
        $test = '<script type = "text/javascript" src = "//ajax.googleapis.com/ajax/libs/jquery/2.0.0/jquery.min.js" ></script><script type="text/javascript" src="' . new moodle_url ( $CFG->wwwroot . $filerelativepath ) . '"></script>';
			return $test;
     ...
     * @return string
     */
    public function block_myteamprogramer_slider_instance_css_as_html($instancecss) {
     ...
     * @return string
     */
    public function block_myteamprogramer_slider_add_instance_css($instancecssid, $displayoptionscss) {
        $instancecss = '';
        ...
        $darkerbackgroundcolor = $this->block_myteamprogramer_slider_alter_brightness ( $displayoptionscss->backgroundcolor, - 50 );
        ...
        $instancecss .= "\n" . $instancecssid . ' .courseslider-course:hover{' . $backgroundcolorcss . '}';
     ...
     * @return string
     */
    public function block_myteamprogramer_slider_courseslider_as_html($mappedcourses, $coursesorder, $instancecssid, $displayoptionshtml) {
        global $CFG;
        $courseslider = '';
        $courseslidernav = '';
        ...
        foreach ($coursesorder as $id) {
            if (array_key_exists ( $id, $mappedcourses )) {
                ...
                $coursepictures = $this->block_myteamprogramer_slider_pictures_as_html ( $course );
                ...
                if ($displayoptionshtml->coursesummaryflag && $course->has_summary ()) {
                    ...
                    $coursehtml .= $this->block_myteamprogramer_slider_summary_as_html ( $coursesummary );
                }
                ...
                if ($displayoptionshtml->coursenameflag) {
                    ...
                    $coursehtml .= $this->block_myteamprogramer_slider_name_as_html ( $coursename );
                    if ($displayoptionshtml->navigationgalleryflag) {
                        $coursenavhtml .= $this->block_myteamprogramer_slider_name_nav_as_html ( $coursename, $courseurl );
                    }
                }
     ...
     * @return string
     */
    public function block_myteamprogramer_slider_name_as_html($coursename) {
        $coursenamehtml = '';
     ...
     * @return string
     */
    public function block_myteamprogramer_slider_name_nav_as_html($coursename, $courseurl) {
        $coursenamehtmlnav = '';
     ...
     * @return string
     */
    public function block_myteamprogramer_slider_summary_as_html($coursesummary) {
        $coursesummaryhtml = '';
        $summarydisplay = '';
     ...
     * @return string
     */
    public function block_myteamprogramer_slider_pictures_as_html($course) {
        ...
        if (empty ( $coursepictures )) {
            ...
            $fs = get_file_storage ();
            $files = $fs->get_area_files ( $context->id, 'block_myteamprogramer_slider', 'defaultimage', false, '', false );
            $file = reset ( $files );
            ...
            if ($file) {
                ...
                $defaultimageurl = $CFG->wwwroot . '/blocks/myteamprogramer_slider/pix/course-slider-default-picture.png';
            }
     ...
     * @return string
     */
    public function block_myteamprogramer_slider_alter_brightness($colourstr, $steps) {
