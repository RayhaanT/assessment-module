 ...
 * @license http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class block_programer_slider_renderer extends plugin_renderer_base {
     ...
     * @return string
     */
    public function block_programer_slider_css_file_as_html($filerelativepath) {
        global $CFG;
     ...
     * @return string
     */
    public function block_programer_slider_js_file_as_html($filerelativepath) {
        global $CFG;
     ...
     * @return string
     */
    public function block_programer_slider_instance_css_as_html($instancecss) {
     ...
     * @return string
     */
    public function block_programer_slider_add_instance_css($instancecssid, $displayoptionscss) {
        $instancecss = '';
        ...
        $darkerbackgroundcolor = $this->block_programer_slider_alter_brightness ( $displayoptionscss->backgroundcolor, - 50 );
        ...
        $instancecss .= "\n" . $instancecssid . ' .coursesliderp-course-image-div{' . $imagedivheightcss . '}';
        $instancecss .= "\n" . $instancecssid . '-nav' . ' .coursesliderp-course-image-div{' . $imagedivheightgallerycss . '}';
        ...
        // Add css for coursesliderp and coursesliderp-nav border color. Leave it to user to make it visible
        // by editing the custom css of the instance.
        $instancecss .= "\n" . $instancecssid . '{' . $bordercolorcsscourse . '}';
        $instancecss .= "\n" . $instancecssid . '-nav' . '{' . $bordercolorcsscourse . '}';

        // Add css for coursesliderp-course,coursesliderp-course:hover,coursesliderp-course-nav.
        $instancecoursecss = $bordercolorcsscourse . $borderradiuscss . $borderstylecss . $borderwidthcss . $boxshadowcss;
        $instancecss .= "\n" . $instancecssid . ' .coursesliderp-course{' . $instancecoursecss . '}';
        $instancecss .= "\n" . $instancecssid . ' .coursesliderp-course:hover{' . $backgroundcolorcss . '}';
        $instancecss .= "\n" . $instancecssid . '-nav' . ' .coursesliderp-course-nav{' . $instancecoursecss . '}';

        // Add css for coursesliderp-course-name.
        $instancenamecss = $backgroundcolorcss . $bordercolorcssname . $colorcss . $borderradiuscss .
                $borderstylecss . $borderwidthcss;
        $instancecss .= "\n" . $instancecssid . ' .coursesliderp-course-name{' . $instancenamecss . '}';

        // Add css for coursesliderp-course-summary.
        $instancesummarycss = $colorcss;
        $instancecss .= "\n" . $instancecssid . ' .coursesliderp-course-summary{' . $instancesummarycss . '}';
     ...
     * @return string
     */
    public function block_programer_slider_courseslider_as_html($mappedcourses, $coursesorder, $instancecssid, $displayoptionshtml) {
        global $CFG;
        $coursesliderp = '';
        $courseslidernav = '';
        ...
        foreach ($coursesorder as $id) {
            if (array_key_exists ( $id, $mappedcourses )) {
                ...
                $coursepictures = $this->block_programer_slider_pictures_as_html ( $course );
                ...
                if ($displayoptionshtml->coursesummaryflag && $course->has_summary ()) {
                    ...
                    $coursehtml .= $this->block_programer_slider_summary_as_html ( $coursesummary );
                }
                ...
                if ($displayoptionshtml->coursenameflag) {
                    ...
                    $coursehtml .= $this->block_programer_slider_name_as_html ( $coursename );
                    if ($displayoptionshtml->navigationgalleryflag) {
                        $coursenavhtml .= $this->block_programer_slider_name_nav_as_html ( $coursename, $courseurl );
                    }
                }

                $coursehtml = html_writer::tag ( 'div', $coursehtml, array (
                        'class' => 'coursesliderp-course',
                        'id' => $courseid
                ) );
                if ($displayoptionshtml->navigationgalleryflag) {
                    $coursenavhtml = html_writer::tag ( 'div', $coursenavhtml, array (
                            'class' => 'coursesliderp-course-nav'
                    ) );
                }

                // Enclose the course in anchor.
                $coursehtml = html_writer::link ( $courseurl, $coursehtml, array (
                        'class' => 'coursesliderp-course-anchor'
                ) );

                $coursecount ++;

                $coursesliderp .= $coursehtml;
                if ($displayoptionshtml->navigationgalleryflag) {
        ...
        $coursesliderp = html_writer::tag ( 'div', $coursesliderp, array (
                'class' => 'coursesliderp',
                'id' => 'coursesliderp' . $instancecssid,
        ...
        if ($displayoptionshtml->navigationgalleryflag) {
            $courseslidernav = html_writer::tag ( 'div', $courseslidernav, array (
                    'class' => 'coursesliderp-nav',
                    'id' => 'coursesliderp' . $instancecssid . '-nav',
                    'data-navigationgallery' => $displayoptionshtml->navigationgalleryflag,
                    'data-numberofslides' => $displayoptionshtml->numberofslides,
                    'data-centermode' => $displayoptionshtml->centermodeflag,
                    'data-navigationoption' => $displayoptionshtml->navigationoptions,
        ...
        return $coursesliderp . $courseslidernav;
    }
     ...
     * @return string
     */
    public function block_programer_slider_name_as_html($coursename) {
        $coursenamehtml = '';

        $coursenamehtml .= html_writer::tag ( 'p', $coursename, array (
                'class' => 'coursesliderp-course-name coursesliderp-truncate'
        ) );
     ...
     * @return string
     */
    public function block_programer_slider_name_nav_as_html($coursename, $courseurl) {
        $coursenamehtmlnav = '';
     ...
     * @return string
     */
    public function block_programer_slider_summary_as_html($coursesummary) {
        $coursesummaryhtml = '';
        $summarydisplay = '';

        $coursesummaryhtml .= html_writer::tag ( 'p', $coursesummary, array (
                'class' => 'coursesliderp-course-summary coursesliderp-truncate'
        ) );
     ...
     * @return string
     */
    public function block_programer_slider_pictures_as_html($course) {
        ...
        foreach ($course->get_course_overviewfiles () as $file) {
            ...
            if ($isimage) {
                $coursepictures .= html_writer::empty_tag ( 'img', array (
                        'src' => $picturesrc,
                        'class' => 'coursesliderp-course-image'
                ) );
            }
        }
        ...
        if (empty ( $coursepictures )) {
            ...
            $fs = get_file_storage ();
            $files = $fs->get_area_files ( $context->id, 'block_programer_slider', 'defaultimage', false, '', false );
            $file = reset ( $files );
            ...
            if ($file) {
                ...
                $defaultimageurl = $CFG->wwwroot . '/blocks/programer_slider/pix/course-slider-default-picture.png';
            }
        ...
        return html_writer::tag ( 'div', $coursepictures, array (
                'class' => 'coursesliderp-course-image-div'
        ) );
    }
     ...
     * @return string
     */
    public function block_programer_slider_alter_brightness($colourstr, $steps) {
