 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class block_programer_slider extends block_base {

    /**
     * Adds title to block instance.
     */
    public function init() {
        $this->title = get_string ( BLOCK_PROGRAMER_SLIDER_BLOCKNAME, BLOCK_PROGRAMER_SLIDER_CLASSNAME );
    }
    ...
    public function specialization() {
        ...
        // Initiate caching.
        $cache = cache::make ( BLOCK_PROGRAMER_SLIDER_CLASSNAME, 'blockdata' );
        ...
        $instancecssid = '#coursesliderp' . $instanceid;
        ...
        $renderer = $this->page->get_renderer ( BLOCK_PROGRAMER_SLIDER_CLASSNAME );
        ...
        if (isset ( $this->config->courses ) && ! empty ( $this->config->courses )) {
            ...
            where u.id = ? and r.shortname='student'";
            ...
            $displayoptionscss->backgroundcolor = get_config( BLOCK_PROGRAMER_SLIDER_CLASSNAME, 'backgroundcolor' );

            if (empty ( $displayoptionscss->backgroundcolor )) {
                $displayoptionscss->backgroundcolor = $defaultblocksettings ['backgroundcolor'];
            }

            $displayoptionscss->color = get_config ( BLOCK_PROGRAMER_SLIDER_CLASSNAME, 'color' );
            ...
            if (empty ( $displayoptionshtml->autoplayspeed )) {
                $displayoptionshtml->autoplayspeed = $defaultinstancesettings ['autoplayspeed'] * BLOCK_PROGRAMER_SLIDER_MILLISECONDS;
            } else {
                $displayoptionshtml->autoplayspeed = $this->config->autoplayspeed * BLOCK_PROGRAMER_SLIDER_MILLISECONDS;
            }

            // Generate coursesliders based on configuration settings.
            $content .= $renderer->block_programer_slider_courseslider_as_html($courses, $coursesorder,
            ...
            $instancecss = $renderer->block_programer_slider_add_instance_css ( $instancecssid, $displayoptionscss );
            ...
            $content .= $renderer->block_programer_slider_instance_css_as_html ( $instancecss );

            // Notice: js and css files do not load for all instances if are not added in the content of the block as done below.

            // Load css files.
            $content .= $this->block_programer_slider_load_css ();

            // Load js files.
            $content .= $this->block_programer_slider_load_js ();
        }
     ...
     */
    private function block_programer_slider_load_css() {
        global $CFG;
        include(dirname ( __FILE__ ) . BLOCK_PROGRAMER_SLIDER_DEFINITIONS);
        ...
        $css .= $renderer->block_programer_slider_css_file_as_html ( $defaultfilerelativepaths ['slickcss'] );
        $css .= $renderer->block_programer_slider_css_file_as_html ( $defaultfilerelativepaths ['slickthemecss'] );

        // Load course slider css.
        $css .= $renderer->block_programer_slider_css_file_as_html ( $defaultfilerelativepaths ['courseslidercss'] );
        $css .= $renderer->block_programer_slider_css_file_as_html ( $defaultfilerelativepaths ['fontawesomecss'] );
        $css .= $renderer->block_programer_slider_css_file_as_html ( get_config ( BLOCK_PROGRAMER_SLIDER_BLOCKNAME, 'customcssfile' ) );
        return $css;
    }
     ...
     */
    private function block_programer_slider_load_js() {
        global $CFG;
        include(dirname ( __FILE__ ) . BLOCK_PROGRAMER_SLIDER_DEFINITIONS);
        ...
        $js .= $renderer->block_programer_slider_js_file_as_html ( $defaultfilerelativepaths ['slickjs'] );

        // Load course slider css.
        $js .= $renderer->block_programer_slider_js_file_as_html ( $defaultfilerelativepaths ['fontawesomejs'] );
        $js .= $renderer->block_programer_slider_js_file_as_html ( $defaultfilerelativepaths ['coursesliderjs'] );
        $customjs = get_config ( BLOCK_PROGRAMER_SLIDER_BLOCKNAME, 'customjsfile' );

        if ($customjs) {
            $js .= $renderer->block_programer_slider_js_file_as_html ( $customjs );
        }
