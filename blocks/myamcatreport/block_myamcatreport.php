    ...
    public function get_content() {
       ...
       // $courseid = 2;
        ...
        $sql = "SELECT am.name, gg.finalgrade FROM {grade_items} as gi 
            LEFT JOIN {amcat} as am on gi.iteminstance = am.id 
            LEFT JOIN {grade_grades} as gg ON gi.id = gg.itemid 
            LEFT JOIN {course_modules} cm on am.id = cm.instance 
            and am.course = cm.course 
             LEFT JOIN {modules} m on m.id = cm.module 
            where itemmodule='amcat' and courseid = ? and gg.userid = ?  and cm.deletioninprogress != 1  and m.name='amcat'";
      ...
      /*  $sqlquiz = "SELECT am.name,  gg.finalgrade FROM {grade_items} as gi 
        ...
        $sqlquiz = "SELECT am.name, gg.finalgrade FROM {grade_items} as gi 
            ...
            and am.course = cm.course
             LEFT JOIN {modules} m on m.id = cm.module 
            where itemmodule='quiz' and courseid = ? and gg.userid = ?  and cm.deletioninprogress != 1  and m.name='quiz'";
        ...
        $coursemoduel = array();
        $finalgrade = array();
		$coursemoduelchart = array();
        foreach ($res as $key => $value) {
            array_push($coursemoduel,substr($value->name,0,3));
			array_push($coursemoduelchart,$value->name);
        ...
        $chtml = $this->chart_model($coursemoduelchart, $finalgrade);
