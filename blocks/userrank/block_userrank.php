...
class block_userrank extends block_base {
    ...
    function get_content() {
		...
		$o.='<button class="btn btn-primary" style="float:right;margin-bottom:5px;" onclick="redir_rank_All();" id="viewAllId"> View All</button><button class="btn btn-primary" style="float:right;margin-bottom:5px;display:none;" onclick="redir_rank();" id="viewlessId"> View Less</button><script>
		window.onload = function() {
			...
			$.post("'.$CFG->wwwroot.'/blocks/ranking/report_glob.php", {"courseid": id,"perpage":"5"}, function (jsondata) {
			$("#rrp").html(jsondata);
			}, "json");
						}
			function redir_rank_All(){
			var id=document.getElementById("ccrid").value;
			...
			$.post("'.$CFG->wwwroot.'/blocks/ranking/report_glob.php", {"courseid": id,"perpage":"All"}, function (jsondata) {
			$("#rrp").html(jsondata);
			}, "json");
						}
			</script>';
			
					$this->title = 'Leader Dashboard';
