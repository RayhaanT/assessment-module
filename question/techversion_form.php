...
class techversion_form extends moodleform
{
    ...
    private function getTopicVersionFields($mform, &$repeatedoptions, $topicnames)
    {
        $repeated = array();

        $repeated[] = $mform->createElement('float', 'version', get_string('topicversiongroup', 'question', $topicnames));

        $repeatedoptions['version']['help'] = array('techversion', 'question');
