   ...
   // $courses = $DB->get_records_sql('SELECT * FROM {course} WHERE category != ?', array(0));
    ...
    if ($DB->get_manager()->table_exists('amcat') && $DB->get_manager()->table_exists('quiz')) {
		...
		$sql_coursemodcompletion = "SELECT count(*) as count FROM {course_modules_completion} AS cmc 
			...
			WHERE cmc.coursemoduleid IN ($coursemod) and userid = ? and completionstate = 1 and (m.name='amcat' OR m.name='quiz')";
		
				
        $mod_completion = $DB->count_records_sql($sql_coursemodcompletion, array($USER->id));
    ...
    if ($DB->get_manager()->table_exists('assign')) {
        ...
        $sql_comp = "SELECT count(*) as count FROM {course_modules_completion} AS cmc 
				...
				ON cmc.coursemoduleid = cm.id 
				INNER JOIN {modules} as m ON m.id=cm.module
				WHERE cmc.coursemoduleid IN ($mcount) and userid = ? and completionstate = 1 and m.name='assign'";
		//print_object($mcount);		
        $completion_assignment = $DB->count_records_sql($sql_comp, array($USER->id));
       // print_object($completion_assignment);exit();
