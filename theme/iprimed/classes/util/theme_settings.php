...
class theme_settings {
    ...
    /**
    * Get config theme footer right 3 icon
    *
    * @return arrray
    */
    ...
    public function footericon(){
        ...
        for ($i = 1; $i < 4; $i++) {
            $footericon = 'footer' . $i . 'icon';
            
            if (!empty($theme->settings->$footericon)) {
                $templatecontext[$footericon] = $theme->setting_file_url($footericon, $footericon);
            }
        }
    ...
    /**
    * Get config theme footer Left icon
    *
    * @return arrray
    */

    public function footerlefticon(){
        $theme = theme_config::load('iprimed');
        $templatecontext = [];
        if (!empty($theme->settings->footer1lefticon)) {
            $templatecontext['footer1lefticon'] = $theme->setting_file_url('footer1lefticon', 'footer1lefticon');
        }
    ...
    public function marketing_items() {
        ...
        for ($i = 1; $i < 6; $i++) {
            ...
            if (!empty($theme->settings->$marketingheading)) {
                $templatecontext[$marketingheading] = theme_iprimed_get_setting($marketingheading, true);
            }

            $templatecontext[$marketingsubheading] = '';
            if (!empty($theme->settings->$marketingsubheading)) {
                $templatecontext[$marketingsubheading] = theme_iprimed_get_setting($marketingsubheading, true);
            }

            $templatecontext[$marketingcontent] = '';
            if (!empty($theme->settings->$marketingcontent)) {
                $templatecontext[$marketingcontent] = theme_iprimed_get_setting($marketingcontent, true);
            }
    ...
    public function numbers() {
        global $DB;
        $theme = theme_config::load('iprimed');
        //$templatecontext['numberusers'] = %;
        $templatecontext['numbersfrontpagetext'] = $theme->settings->numbersfrontpage;
        return $templatecontext;
    }
    ...
    public function clients() {
        ...
        for ($i = 1, $j = 0; $i <= $clientscount; $i++, $j++) {
            ...
            $templatecontext['clientsimage'. $j]= $image;
