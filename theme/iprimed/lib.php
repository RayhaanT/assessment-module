 ...
 * @return string
 */
function theme_iprimed_get_extra_scss($theme) {
    $scss = $theme->settings->scss;

    $scss .= theme_iprimed_set_headerimg($theme);

    $scss .= theme_iprimed_set_topfooterimg($theme);

    $scss .= theme_iprimed_set_loginbgimg($theme);
 ...
 * @return string
 */
function theme_iprimed_set_headerimg($theme) {
    global $OUTPUT;
 ...
 * @return string
 */
function theme_iprimed_set_topfooterimg($theme) {
    global $OUTPUT;
    ...
    if (!is_null($topfooterimg)) {
        ...
        $headercss = '#top-footer {background-color: '.$footercolor.';}';
    }
 ...
 * @return string
 */
function theme_iprimed_set_loginbgimg($theme) {
    global $OUTPUT;
    ...
    $headercss = "#page-login-index.iprimed-login #page-wrapper #page {background-image: url('$loginbgimg');}";
 ...
 * @return string
 */
function theme_iprimed_get_main_scss_content($theme) {
    global $CFG;
    ...
    } else if ($filename && ($presetfile = $fs->get_file($context->id, 'theme_iprimed', 'preset', 0, '/', $filename))) {
        // This preset file was fetched from the file area for theme_iprimed and not theme_boost (see the line above).
    ...
    // iprimed scss.
    $iprimedvariables = file_get_contents($CFG->dirroot . '/theme/iprimed/scss/iprimed/_variables.scss');
    $iprimed = file_get_contents($CFG->dirroot . '/theme/iprimed/scss/iprimed.scss');

    // Combine them together.
    return $iprimedvariables . "\n" . $scss . "\n" . $iprimed;
}
 ...
 * @return string
 */
function theme_iprimed_get_pre_scss($theme) {
    $scss = '';
    $configurable = [
 ...
 * @return mixed
 */
function theme_iprimed_pluginfile($course, $cm, $context, $filearea, $args, $forcedownload, array $options = array()) {
    $theme = theme_config::load('iprimed');
 ...
 * @return string
 */
function theme_iprimed_get_setting($setting, $format = false) {
    $theme = theme_config::load('iprimed');
...
/**
 * Extend the iprimed navigation
 *
 * @param flat_navigation $flatnav
 */
function theme_iprimed_extend_flat_navigation(\flat_navigation $flatnav) {
    theme_iprimed_rebuildcoursesections($flatnav);

    theme_iprimed_delete_menuitems($flatnav);
}
 ...
 * @param flat_navigation $flatnav
 */
function theme_iprimed_delete_menuitems(\flat_navigation $flatnav) {
 ...
 * @param flat_navigation $flatnav
 */
function theme_iprimed_rebuildcoursesections(\flat_navigation $flatnav) {
    global $PAGE;
    ...
    if ($PAGE->course->format != 'singleactivity') {
        $coursesectionsoptions = [
            'text' => get_string('coursesections', 'theme_iprimed'),
            'shorttext' => get_string('coursesections', 'theme_iprimed'),
